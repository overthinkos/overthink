export const meta = {
  ...
  description:
    'Evaluate deployment configs — for AI agents and human operators. Runs `ov image validate` once (config correctness), then per target a read-only deploy-verifier pass (`ov eval image` for built images, `ov eval live` + `ov status` for running deploys) and aggregates a health report. NON-destructive: never builds, deploys, rebuilds, or tears down. For the destructive R10 bed gate use /verify-beds instead.',
  phases: [
    { title: 'Validate', detail: 'ov image validate — config + warnings' },
...
const validation = await agent(
  'Run `ov image validate` in this project. Return {ok, warnings, errors} — list every warning and error verbatim. Do not build or deploy anything.',
  { schema: VALIDATE_SCHEMA, label: 'ov image validate', phase: 'Validate' }
)
if (validation && validation.warnings && validation.warnings.length) {
  log(`ov image validate: ${validation.warnings.length} warning(s) — these block a clean R10 (zero-warnings gate).`)
}
...
if (requested.length) {
  ...
  const discovered = await agent(
    'Read overthink.yml / image.yml in this project. Return JSON {targets:[{name,kind}]} listing the ENABLED image short-names (kind "image") and any deploy names from a local deploy.yml (kind "deploy"). Do NOT run or build anything.',
...
if (!targets.length) {
  log('No deploy targets discovered — only ov image validate ran.')
...
const results = (
  await parallel(
    targets.map((t) => () =>
      agent(
        `You are the deploy-verifier (read-only — never build/deploy/rebuild/destroy). Evaluate the deploy config "${t.name}" (kind ${t.kind}). Run the probes that apply: \`ov eval image ${t.name}\` if a built image exists locally (use the full registry ref if the short name is ambiguous; report NOT-BUILT if absent), and \`ov eval live ${t.name}\` + \`ov status ${t.name}\` if it is currently deployed (report NOT-RUNNING if not). Return the health verdict with verbatim failing-check output — never hide a failed check.`,
