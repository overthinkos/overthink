...
const repro = await agent(
  `You are an eval-failure triager. The kind:eval bed "${bed}" failed. Read .eval/${bed}/<calver>/summary.yml (newest run dir) and tail the failing step's .log. Report the failing step, the process exit code, the log tail, and the concrete observed symptom. Do NOT mutate anything, do NOT re-run the bed.`,
...
log(`triage-eval-failure(${bed}): ${hyps.length} hypotheses, ${survivors.length} survived adversarial cross-check.`)
