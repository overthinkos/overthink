...
1. **VM autostart** — new `VmSpec.Autostart` field (`ov/vm_spec.go`).
   ...
   ov VMs run under `qemu:///session`, calls `ensureBootAutostartPrereqs`
   (`ov/vm.go`): idempotent `loginctl enable-linger <user>` + `systemctl --user
   enable virtqemud.socket`. The flag is a libvirt domain property (not XML), so it
   survives redefinition and is re-asserted on every create/rebuild.
