...
- **Deploy substrates** — a deploy's substrate kind at the edge picks where it lands: `local:` (direct shell, or SSH when `host:` names a machine), `vm:`, `k8s:`, `android:`, or `pod:` (the default) running a box `image:` — all consuming the shared InstallPlan IR. See `/charly-core:deploy`, `/charly-internals:install-plan`.
