...
Secure the box, then fill it with the whole candy store: compose, build, deploy, and manage **boxes** (container images) from a library of fully configurable **candies** (layers). Built on a generic init system framework (`build.yml` → `init:` section) and `charly` (Go CLI). Designed to work equally well from the command line and from AI agents like Claude Code — for you *and* your agents. Supports both Docker and Podman.
