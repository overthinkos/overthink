...
- **Tests ship with the image — MANDATORY per candy.** Every candy MUST carry a non-empty `description.feature:` AND a top-level `scenario:` list with ≥1 deterministic `do: assert` step (the unified `Op` vocabulary — verb × `do:` × `context:` — folded the former separate `eval:` check list into `scenario:`); `charly box validate` hard-errors otherwise. See `/charly-eval:eval`. **Per-box `eval_level:`** (`none`|`build`|`noagent` (default)|`agent`) gates how deep `charly eval run <bed>` drives acceptance. See `/charly-image:image`.
