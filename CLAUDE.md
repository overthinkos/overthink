...
**Anti-patterns that FAIL the cutover** — beyond the R2–R5 anti-patterns (pre-existing/out-of-scope classification → R2; band-aid duplication → R3; ad-hoc workarounds → R4; stale references after deletion → R5; all catalogued in `/ov-internals:strict-policy`), these are cutover-specific:
...
These rules cover the gap between "R10 verified" and "user picks up the next task". Every step is sequential — do them in order. The deep landing mechanics (CalVer tag computation, multi-repo push order, the fork+PR path, `gh` approve/merge) live in `/ov-internals:git-workflow`; this section is the gate.
...
3. **Auto-land the `feat/` branch — gated by R10, NEVER force-push.** The change was developed on a `feat/<slug>` branch off up-to-date `main` (see `/ov-internals:git-workflow`). The **R10 pass is the sole landing trigger** — there is no per-change human "push" step (this SUPERSEDES the older "push only if the user asked"). On R10 PASS, automatically and in order: (a) push `feat/<slug>`; (b) `git merge --ff-only feat/<slug>` into `main` (if `main` advanced, re-sync, rebase `feat/` onto it, and re-run R10 first); (c) tag the new `main` HEAD with a fresh `v<CalVer>` and push `main --follow-tags`; (d) delete `feat/<slug>` local + remote. NOTHING is ever pushed or merged on unverified state. **NEVER force-push** — no `git push --force`, no `--force-with-lease`, on ANY branch (`feat/` included) in ANY repo, ever; `main` only fast-forwards, tags are immutable/add-only. The whole flow is designed so a force push is never needed. Tag computation, multi-repo dependency order (submodule → `plugins` → superproject), cross-repo `@github` landing, and the no-write-access fork+PR path are all in `/ov-internals:git-workflow`.
4. **Eval-coverage gate.** R10 does not pass — and the change is NOT landable — unless it ships the test coverage that PROVES its new functionality (`eval:` checks for new/changed layers & images; Go tests for `ov` code) AND the live run exercised it. See R7/R10, `/ov-eval:eval`.
5. **Zero-warnings gate.** R10 is NOT successful while ANY warning remains — resolver newest-wins, build, `ov box validate`, `ov eval`, or deploy warnings. Each is fixed before R10 passes: a version-mismatch warning is cleared with `ov box reconcile`; any other warning triggers `/ov-internals:root-cause-analyzer` then a real fix. A surviving warning is an R10 failure, never an accepted end state (this is R1 made a hard gate).
...
1. **Run `/ov-internals:root-cause-analyzer` BEFORE attempting any fix.** Blind retry is FORBIDDEN. R10 caught a real regression; understand it first.
...
3. **Re-run R10 from scratch.** Not just the failing piece — the FULL R10 against a fresh `ov update`. A fix that survives only the targeted re-run but breaks something else is a regression in waiting.
...
- [ ] If you broke the target during exploration, you `ov update`d it back to clean before continuing?
...
- **Agent roster** (`plugins/internals/agents/`) — *executors* run `ov eval` and return verbatim proof: `eval-bed-runner` (full `ov eval run <bed>`, R10 acceptance discipline), `deploy-verifier` (read-only `ov eval box`/`live` + `ov status`). *Enforcers* gate claims: `root-cause-analyzer` (R1 RCA), `testing-validator` (proof-before-"works"), `layer-validator` (pre-edit `candy.yml`).
...
- **Engineering discipline → runtime verification → acceptance** → **Ground Truth Rules** R1–R5 (RCA / no-deferral / no-duplication / no-workarounds / hard-cutover) before R6–R9 before R10. Operationalized in `/ov-internals:strict-policy`.
...
- **Hard cutover by default** → **Hard Cutover by Default** + `/ov-internals:cutover-policy`.
- **Branch-per-change, R10-gated auto-landing, NEVER force-push** → **Post-Execution Policies** + `/ov-internals:git-workflow`.
- **Tag every push with a fresh CalVer timestamp** → **Post-Execution Policies** (`v$(date -u +%Y).$((10#$(date -u +%j))).$(date -u +%H%M)`, day-of-year NOT zero-padded; ONE immutable add-only tag per push; INDEPENDENT of the `overthink.yml` `version:` schema field — and a YAML schema/format change does BOTH: raise `LatestSchemaVersion()` via a `MigrationStep` AND carry the fresh tag on the landing push; `plugins`/`pkg/arch` are tag-exempt). Detail: `/ov-internals:git-workflow`, `/ov-build:migrate`.
- **Every change ships proof of its functionality** → the eval-coverage gate in **Post-Execution Policies** / R7 / R10. A change whose new functionality has no test that would FAIL without it is not landable. See `/ov-eval:eval`.
