...
| Editing a box (`box/<name>/charly.yml` — boxes live in the `image/<distro>` submodules; main owns none after the box inversion), box composition | `/charly-image:image` |
...
- **Per-directory CLAUDE.md signposts.** This root file is the single canonical R0–R10 rule-set. `charly/`, `candy/`, `plugins/`, and each `image/<distro>` submodule carry a THIN signpost `CLAUDE.md` that only names the skills to load for that area and points back here — it restates no rule (duplication drifts). Subagents/teammates load the full `CLAUDE.md` hierarchy from their working dir.
