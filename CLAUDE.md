...
**What is NOT post-execution:** folding new work INTO the current cutover is FORBIDDEN — but STARTING the next one is the AI's job: the moment this cutover lands, the AI automatically opens the next cutover for ANY issue it has found (backports and cherry-picks included), each its own atomic, fully-R10'd change, pausing to ask ONLY at a genuine crossroad it cannot resolve from the request, the code, the loaded skills, or sensible defaults. "Phase 2" is a forbidden concept.
