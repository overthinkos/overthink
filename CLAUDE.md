...
4. **Eval-coverage gate.** R10 does not pass — and the change is NOT landable — unless it ships the test coverage that PROVES its new functionality (`eval:` checks for new/changed layers & images; Go tests for `charly` code) AND the live run exercised it. See R7/R10, `/charly-eval:eval`.
