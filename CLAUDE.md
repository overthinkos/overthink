...
| **— Evaluate & verify —** | |
| `charly check *` (ANY check verb, incl. `charly check box`) / `charly check run <bed>` (kind:check R10 bed) / authoring `kind: check` beds / `charly check live` / the probe verbs (cdp/wl/dbus/vnc/mcp/record/spice/libvirt) / `recipe:`/`score:` AI-agent scoring / `charlycheck/*` branches | `/charly-check:check` |
| Agent Driven Evaluation (ADE) / `charly box feature run` / `charly check feature run` / `charly feature list/pending/validate` / `charly candy add-scenario` / `description:` Gherkin scenarios / the agent grader for prose steps | `/charly-check:check` + `/charly-internals:strict-policy` |
| `charly check k8s <verb>` / cluster probes | `/charly-kubernetes:check-k8s` |
| `charly check adb <method>` / Android Debug Bridge from host (devices, shell, install, getprop, screencap, logcat, wait-for-device) | `/charly-check:adb` + `/charly-check:check` |
| `charly check appium <method>` / Android UI automation / W3C WebDriver sessions, element introspection, the gesture/app/key/device sugar groups, the generic `execute`/`raw` escape hatch | `/charly-check:appium` + `/charly-check:check` |
...
Every entity's intended behaviour is captured as executable Gherkin scenarios (a `description:` block + a top-level `scenario:` list), authored by you OR your agents, baked into the image as the `ai.opencharly.description` OCI label, and runnable as acceptance tests: **the spec IS the test**. A scenario `step:` is one `Op` — a verb on one of three axes: `do: assert` (the deterministic goss-style check — `command`/`file`/`port`/`http`/`cdp`/…, run standalone with no agent), `do: act` (the same verbs used to CONFIGURE — install a package, drive a UI), or `do: instruct` (the `agent:` verb — free-form prose handed to an AGENT grader probing the live deployment; an unparseable / timed-out grader FAILS the step, never a silent pass). Scenarios live on the CANDY that provides the behaviour, so ONE scenario covers every box composing it (R3). ADE is MANDATORY for every candy: each candy MUST ship a non-empty `description.feature:` AND a `scenario:` list with **at least one deterministic `do: assert` step** — `charly box validate` hard-errors otherwise — and the baked scenarios run and must pass. RDD proves the assumptions, ADE specifies and grades the behaviour, R10 proves the fresh rebuild — three points on one *never trust, verify* arc.
...
- **R10. Verify on a `disposable: true` target; prove it on a fresh rebuild.** Test ONLY on targets explicitly marked `disposable: true` (none suitable → create one first; never assume disposability from a name, lifecycle tag, or hostname); if a test breaks the target, `charly update` it back to committed config before anything else; after committing the fix, re-verify on a FRESH `charly update` — pasted fresh-rebuild output, at ZERO warnings, with the check/test coverage that proves the new functionality, is the acceptance gate.
  *Fraud clauses — each a hard violation (motivating incidents: `CHANGELOG.md`):*
  ...
  - **Flag overrides require explicit user authorization in the SAME turn.** The score/bed config in the `check:` block IS the test specification; passing ANY scope-shrinking `charly check run`/`live` flag without the user naming the flag THIS turn is the same fraud class (authoritative catalog: `/charly-check:check` "Flag discipline"); "to fit session bounds" is a confession, not a defence.
...
- **Deploy fetches NOTHING speculative.** `charly deploy add` (any target kind) emits zero image-pull / image-build steps; test-bed image preflight belongs to the check entry point. See `/charly-local:local-spec`, `/charly-check:check` "Image preflight".
...
- **Cross-deployment probing via `peer:` siblings — ONE deployment tests ANOTHER** over the shared `charly` network (`on: <peer>`, `${PEER_HOST:…}` / `${PEER_ENDPOINT:…}`); peers inherit the owner's disposability and are never check-live'd. See `/charly-check:check` "Cross-deployment probing", `/charly-core:deploy` "Sibling peers".
- **Android is a first-class deploy substrate** — a `kind: android` entity is a DEVICE; `apk:` is the candy-declared package format ONLY `target: android` executes; `pod → android` nesting mirrors `vm → k8s`. See `/charly-check:android`, `/charly-check:adb`, `/charly-check:appium`.
