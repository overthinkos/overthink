...
| Agent Driven Development (ADD) / `ov box feature run` / `ov eval feature run` / `ov feature list/pending/validate` / `ov candy add-scenario` / `description:` Gherkin scenarios / the agent grader for prose steps | `/ov-eval:eval` + `/ov-internals:strict-policy` |
...
| `ov box reconcile` / cross-repo `@github` pin alignment / layer-version-mismatch cleanup | `/ov-build:reconcile` |
...
- **R7. Unit tests never substitute for runtime verification — mandatory end-to-end gate.** A green `go test ./...` means the code compiles and fixture loaders work — nothing about whether the produced artifact behaves correctly. For any change that can affect Containerfile generation, OCI labels, init systems, service startup, or deploy code, the minimum sequence applies BEFORE "done":
  1. `ov box build <image>` — build a concrete image (not just generate Containerfile).
  2. `ov eval box <image>` — baked layer + image sections pass (NB: passes on zero-content stages too — not a substitute for R8).
  ...
  **Which eval verb for R10 — pick by what you're proving:**
  - `ov eval box <image>` — build-scope invariants only (binary / package presence) in a disposable `podman run --rm`. No deploy, no live runtime state.
...
See `/ov-internals:disposable` (the lifecycle boundary), `/ov-eval:eval` (the `ov eval` candy store + the disposable bed), and `/ov-internals:agents` (agents and teams working inside the box).
...
Overthink is built around **Agent Driven Development (ADD)**: every entity's intended behaviour is captured as executable Gherkin scenarios (a `description:` block — Feature / Narrative + Given/When/Then steps), authored by you OR your agents, baked into the image as the `org.overthinkos.description` OCI label, and verified on every build. The spec IS the test; agents are first-class AUTHORS of it and first-class GRADERS of it. ADD is the canonical Gherkin acceptance-testing pattern, named for the agent that drives it — the natural fit for a system built "for you and your agents".
...
5. **Bake** — `ov box build` bakes goal + scenarios into `org.overthinkos.description`; the artifact carries its own runnable acceptance spec (source-less `ov box`/`ov eval feature run` against a pulled image).
6. **Gate** — `ov eval run <bed>` runs the bed image's deterministic scenarios as an opt-in acceptance gate (a no-op PASS when none are authored).
...
- **Running `ov` to verify, at any stage, as often as useful.**
  `ov box build`, `ov update`, `ov eval run`, `ov vm create`, `ov start`
...
3. **Verify continuously; the commit gate is the full final-code test.**
   Run `ov` commands freely throughout — `ov box build`, `ov update`,
   ...
   `go test`, `ov box validate`) is good hygiene between tasks but is NOT
   the gate.
...
3. **Auto-land the `feat/` branch — gated by R10, NEVER force-push.**
   ...
   - **Multi-repo / cross-repo:** one logical change uses the SAME
     ...
     `@github` lands the producer + tag FIRST, then `ov box reconcile`
   ...
   - **Zero-warnings gate.** R10 is NOT successful while ANY warning
     remains — resolver newest-wins warnings, build, `ov box validate`,
     `ov eval`, or deploy warnings. Each is fixed before R10 passes: a
     version-mismatch warning is cleared with `ov box reconcile`; any
...
a long background task whose OWNER must survive across turns to be notified on
completion** (launched `run_in_background`; uncapped — 600s is a Bash FOREGROUND
cap, irrelevant to a backgrounded bed). **Verified owners (2026-06):** the
**persistent main session** (one `run_in_background` task per bed — the headless
default), a **background agent** (`Agent` run_in_background — per-bed
out-of-process, works headless), or a **split-pane teammate** (interactive tmux
only). An **in-process teammate CANNOT** own a bed — its bg task is torn down the
instant it yields (verified 4×); it does bed-local edits + short foreground checks
only. So **"one agent ⇄ one bed" = one persistent owner per bed.** The lead
partitions the `kind: eval`
...
overlap only fails the second bed at deploy), so beds run concurrently with no
...
wall-clock ≈ the slowest single bed, not the sum. IN-PROCESS teammates do NOT run
full beds (their bg dies on yield) — they do bed-local edits + short foreground
checks; a PERSISTENT owner (main session / background agent / split-pane teammate,
per above) runs each full bed and triages failures. One cutover stays one phase / one commit, owned by the
...
checks) touches no shared binary → NO barrier → owners iterate their beds fully in
parallel; only a SHARED-CORE change (`ov/*.go`) needs synchronization, because
`ov`'s stale-binary freshness guard gates EVERY heavy verb the instant any
`ov/*.go` is newer than `/usr/bin/ov`. Single-Go-package compile-coherence is then
handled STRUCTURALLY — the lead lands the shared core FIRST, each parallel unit is
an independent `init()`-registered file, and the one shared `ov` binary rebuild is
a single barrier between parallel-implement and parallel-bed-R10, **with Go FROZEN
for the bed phase** (a teammate editing `ov/*.go` mid-bed-run trips the guard and
aborts every other agent's next heavy verb) — it is NEVER a license to serialize
the whole implementation or to trade beds for review. Canonical shape: `Core (seq) → Implement (parallel by
...
<bed>`; a **persistent owner runs every full bed as a `run_in_background` task**
— main session / background agent / split-pane teammate, NOT an in-process
teammate — and pastes the verdict; teammates do bed-local edits + short checks),
`deploy-verifier` (read-only `ov eval box`/`live` + `ov status` for an image
...
(validate + `ov eval box`/`live` + `deploy-verifier`) for AI and humans.
...
task** (`run_in_background`) — never in the foreground (the Bash tool's `timeout`,
120s default / 600s max, kills the call mid-`vm-create`, orphaning the domain) and never kept
...
**every full `ov eval run <bed>` belongs to a PERSISTENT owner as a
`run_in_background` task** (main session / background agent / split-pane teammate;
never an in-process teammate) — duration-independent (the 600s is a Bash
FOREGROUND cap, irrelevant to a backgrounded bed); sub-agents/in-process teammates do bed-local edits + short
...
R0–R10 rule-set. `ov/`, `candy/`, `plugins/`, and each `image/<distro>`
...
**Forward-looking thesis lives in `VISION.md`** (repo root). The long-term "why this exists and where it's going" — distilled from the philosophy pillars (Candyboxing, RDD, Agent Driven Development, Disposable-Only Autonomy, "for you and your agents") — is stated there as ASPIRATION, in present-and-future tense. `VISION.md` never restates command usage (→ `README.md` / skills), architecture / subsystem detail (→ skills), or history (→ `CHANGELOG.md`); it links out to them. This makes the doc split five-way: rules → CLAUDE.md, features/commands → README.md, usage/architecture → skills, history → CHANGELOG.md, thesis/direction → VISION.md.
...
- **Unified YAML + `import:` (Go-style namespaces).** `overthink.yml` is the single project entry point. The SINGLE composition statement is `import:` (the legacy `include:` key is DELETED — a residual `include:` is a hard load-time error pointing at `ov migrate`). `import:` is a LIST whose items are either a **bare string** (flat import into THIS repo's root namespace — same-repo per-kind files + the shared `build.yml` distro/builder/init vocabulary) or a **single-key map `alias: ref`** (a namespaced child import of another project; entries referenced QUALIFIED as `alias.entry`, e.g. `base: cachyos.cachyos`, `builder: {pixi: ov.arch-builder}`). Resolution is namespace-relative (Go package-member semantics); `distro:`/`build:` inherit across a namespace boundary but `builder:` does NOT (the consumer declares its own builder map). The **main repo stays multi-file** (`base.yml` = arch+fedora stacks, plus `eval.yml`/`box.yml`/`build.yml`/…); **each `image/<distro>` submodule is its own ov-project, structured like the main repo** — an `overthink.yml` entry point that flat-imports its per-kind sibling files (`box.yml`/`pod.yml`/`k8s.yml`/`vm.yml`) where present (most submodules) OR inlines them all in the one `overthink.yml` (e.g. `bootc`); both layouts load identically (the `import:` list flat-merges siblings root-wins). Each imports main under the `ov` namespace and `build.yml` flat. The main↔cachyos mutual import is cycle-broken at load. See `/ov-image:image`, `/ov-internals:go`, `/ov-build:migrate`.
- **Schema — every YAML file is a GENERIC kind-container, routed by SHAPE.** The kinds (`box`, `candy`, `pod`, `vm`, `k8s`, `local`, `android`, `deploy`, plus the build-vocabulary kinds `builder`/`distro`/`init` and the eval kinds `ai`/`recipe`/`score`/…) are `kind:` discriminators. ANY file may hold ANY mix of kinds — the loader routes each document by its top-level kind-key, **NEVER by filename**. A per-kind sibling file (`box.yml` holding boxes, `candy.yml` holding candies, …) is a pure user **CONVENIENCE** expressed in `overthink.yml`'s `import:` / `discover:`; it is never required, assumed, or hardcoded. **`overthink.yml` is the ONLY YAML filename the code knows** — every other name is configured there. `discover:` is a **FLAT generic scan-spec list** (`- {path, recursive, manifest}`): each spec scans a path for its manifest and routes every discovered document by shape, with the manifest filename configured per spec (one overridable `DefaultManifest` default). The schema version is a CalVer string (e.g. `2026.156.1041`), the same scheme as image tags; configs older than `LatestSchemaVersion()` migrate via the single idempotent `ov migrate`. Nesting of deployments uses `nested:`. See `/ov-build:migrate`, `/ov-image:image`, `/ov-internals:go`, `/ov-core:deploy`, `/ov-vm:vm`, `/ov-local:local-spec`, `/ov-eval:android`.
...
- **Candyboxing, not sandboxing.** Secure the BOX as a whole — a disposable container / VM / eval bed with a real, kernel-enforced boundary — then fill it with the FULL candy store (every `ov` verb, MCP server, layer, and `ov eval` probe) so the AI is never artificially limited in what it can build, deploy, or test. The generosity inside the box is what makes RDD honest and disposable autonomy fearless; it never widens the boundary or loosens the safety gates. See the **Candyboxing** section above.
...
- **Agent Driven Development (ADD) — the spec is the test; agents author and grade it.** Capture an entity's intended behaviour as executable Gherkin scenarios on the LAYER that provides it (`description:` Feature/Narrative + Given/When/Then), authored by you or an agent (`ov candy add-scenario`). A step binds BY SHAPE: embed a check verb → deterministic; leave it prose → agent-graded. Run them with `ov box feature run <image>` (build) / `ov eval feature run <deployment>` (deploy, prose steps agent-graded against the live system), drive them to green by hand or via the `ov eval run <score>` AI loop, and they bake into `org.overthinkos.description`. Co-equal pillar with RDD; an OPT-IN runnable gate (`ov eval run <bed>` runs baked scenarios, no-op when none). See the **Agent Driven Development** section above and `/ov-eval:eval`.
