...
When services run as separate containers, **service discovery happens automatically**. A layer can declare `env_provides` — environment variables (with `{{.ContainerName}}` templates) that get injected into all other deployed containers at `ov config` time. For example, deploying `ollama` automatically provides `OLLAMA_HOST=http://ov-ollama:11434` to every other container — no manual environment setup needed. Layers can also declare `env_requires` (mandatory env vars like API keys) and `env_accepts` (optional env vars) for documentation and deploy-time validation.
...
- **`layer.yml`** — The layer's manifest: system packages with tag-based dispatch (`rpm:` for Fedora/RHEL, `deb:` for Debian/Ubuntu, `pac:` for Arch Linux, `aur:` for AUR, plus distro/version tags like `fedora:`, `fedora:43:`), dependencies on other layers, environment variables, cross-container env injection (`env_provides`), env var requirements (`env_requires`, `env_accepts`), ports, services, volumes, routes, and metadata (`version`, `status`, `info`)
