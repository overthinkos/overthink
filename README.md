...
`/ov-openclaw:openclaw-desktop` (streaming-desktop variant) for
canonical compositions.

### Sandboxed AI Desktops

One of Overthink's design goals is running sandboxed [OpenClaw](https://github.com/overthinkos/openclaw) systems. The approach flips the usual AI sandboxing model: instead of restricting what the AI agent can do, Overthink gives it full access to a complete desktop environment — Chrome, a Wayland compositor, development tools, network services — and sandboxes the entire desktop inside a container managed by `ov`. The AI agent operates freely within its environment while the host stays fully isolated. This is how an image like `sway-browser-vnc` (a sandboxed Sway + Chrome + VNC desktop) works — compose the `openclaw` gateway layer on top for a full AI workstation with no host compromise.

`/ov-openclaw:openclaw-desktop` takes this a step further: the all-in-one CachyOS streaming desktop fuses the selkies desktop, the openclaw-full gateway + AI CLIs (claude-code/codex/gemini), a CPU ollama, and the full `ov` toolchain — so the AI (or the user) can build images, launch nested rootless pods, and create libvirt VMs from a terminal inside the browser-accessible desktop — all at uid 1000 with no `--privileged` and no added capabilities. The rootless-in-rootless recipe (kernel `mount_too_revealing()` RCA, surgical `unmask=/proc/*`, `virtqemud` session daemon) is documented in `/ov-distros:container-nesting` and `/ov-infrastructure:virtualization`.
...
Running images ship comprehensive coverage: `filebrowser` (24), `jupyter` (32), `openwebui` (24), `hermes` (50), `immich-ml` (63), `selkies-desktop` (91), `sway-browser-vnc` (92), `openclaw-desktop` (135 image-scope · 168 live-service). The jupyter and sway-browser-vnc counts include the `mcp:` declarative checks (3 and 2 respectively) introduced with the `ov eval mcp` verb. `openclaw-desktop` — the all-in-one CachyOS desktop fusing the selkies stream, the openclaw-full gateway + AI CLIs, a CPU ollama, and the nested ov toolchain — adds extra gates for the nested-podman recipe: its live-service run (`ov eval live openclaw-desktop`) verifies nested `podman run quay.io/libpod/alpine:latest`, the `virsh -c qemu:///session` rootless session, and in-container `ov version` / `ov doctor`. LABEL directives emit at the end of each Containerfile so test edits rebuild in ~2 seconds.
...
VM creation also works **rootless** via `qemu:///session` and a supervisord-managed `virtqemud` daemon, so `ov vm create` runs from inside a rootless container (e.g., `/ov-openclaw:openclaw-desktop`) as uid 1000 with only `/dev/kvm` passthrough — no root, no `--privileged`, no `CAP_SYS_ADMIN`. See `/ov-infrastructure:virtualization` for the supervisord program definitions and the session-mode setup.
