// Command plugin-builder-pixi is the OUT-OF-TREE charly plugin serving the `pixi` builder's
...
// the compiler pure. The per-builder LOGIC is the shared charly/plugin/kit (R3 — one source for all
// four detection-builders); this module is only the composable selection point + serve shim.
package main
...
// Describe advertises the builder:pixi capability + its self-contained CUE schema over the same
// channel a builtin uses; BuildCapabilities compiles the schema standalone, failing loudly if broken.
func (meta) Describe(context.Context, *pb.Empty) (*pb.Capabilities, error) {
	return sdk.BuildCapabilities("2026.181.0100",
