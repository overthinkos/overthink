...
//
// clean is a COMMAND-class plugin: charly dispatches it by fork/exec'ing this binary in CLI
...
// #CleanPlugin documents the command the plugin serves. The command keeps its entire contract
// in the in-core CleanCmd grammar (the `charly __clean` leaf it raw-forwards to), so there is
// no plugin_input to validate here.
#CleanPlugin: {
	command:  "clean"
	contract: "clean is CLI-dispatched (charly fork/execs the binary); args are plain CLI tokens that raw-forward to the in-core build-artifact retention/prune command via charly __clean"
}
