...
//
// feature is a COMMAND-class plugin: charly dispatches it by fork/exec'ing this binary in CLI
...
#FeaturePlugin: {
	command:  "feature"
	contract: "feature is CLI-dispatched (charly fork/execs the binary); args are plain CLI tokens (list [kind] | pending [entity] | validate [entity]) that shell back to the in-core loader + plan model via charly __feature-list / __feature-pending / __feature-validate"
}
