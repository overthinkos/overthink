...
// plugin. The seven InstallStep kinds it serves (file / shell-hook / shell-snippet /
// service-packaged / service-custom / repo-change / apk-install) are COMPILER-EMITTED from
// declarative candy fields (copy:/env:/service:/…), never authored as a `plugin:` step, so NO
// capability declares an InputDef. This def exists ONLY to satisfy the plugin load gate (every
