...
func cutover4HostState(ctx *MigrateContext) ([]string, error) {
	...
	// charly/<svc>. List/Get/Set/Delete on the host store, which charly core INJECTS
	// (DefaultCredentialStore is package-main, unreachable from this candy). A nil
	// provider (the store was never injected — e.g. a unit test) skips the re-key;
	// a backend that cannot enumerate (returns an error) is skipped, not fatal.
	if credentialStoreProvider != nil {
