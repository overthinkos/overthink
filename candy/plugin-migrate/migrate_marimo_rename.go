...
// migrate_marimo_rename.go — `charly migrate`.
//
...
// containers under the OLD deploy names so a follow-up `charly update`
...
func MigrateMarimoRename(dir string, dryRun bool) ([]string, error) {
	...
	// `charly update <new>` would fail trying to rewrite a quadlet at the
	// new path. Stop + remove both surfaces so the next `charly config` /
	// `charly update` regenerates them cleanly. Best-effort: each step
...
//
// Sequence (mirrors what `charly remove` does, minus the deploy.yml edit
