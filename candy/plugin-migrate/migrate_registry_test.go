...
func TestMigrateCalverSchema_StampsAndIdempotent(t *testing.T) {
	dir := t.TempDir()
	overthink := filepath.Join(dir, "charly.yml")
	...
	if len(baks) != 1 {
		t.Errorf("expected 1 backup for charly.yml, got %d", len(baks))
	}
...
func TestMigrateCalverSchema_EdgeCases(t *testing.T) {
	...
	legacy := filepath.Join(dir, "charly.yml")
...
// TestLoadUnified_CalVerGateRejectsLegacy moved to charly core
// (charly/migrate_calver_gate_core_test.go) — it exercises the CORE LoadUnified
// schema gate (package-main), not the registry (C13a).
