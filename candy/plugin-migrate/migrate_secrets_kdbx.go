...
func MigrateDropKdbx(path string, dryRun bool) ([]string, error) {
	if path == "" {
		// The registry always passes ctx.HostConfigPath (resolved by core's
		// NewMigrateContext via RuntimeConfigPath, package-main). An empty path means
		// there is no per-host config to clean — nothing to migrate.
		return nil, nil
	}
