...
// MigrateSingleFilename is the public/test form. Whether a local build.yml matches
// the embedded vocabulary (mergeUnifiedDocs + embeddedDefaults, both package-main)
// is HOST-PRELIFTED by charly core; the registry passes ctx.BuildYmlMatchesEmbed to
// migrateSingleFilename. This form supplies false (a customized build.yml is kept),
// the conservative default.
func MigrateSingleFilename(dir, hostDeployPath string, dryRun bool) ([]string, error) {
	return migrateSingleFilename(dir, hostDeployPath, dryRun, false)
}

// migrateSingleFilename applies the single-filename cutover to a project tree.
// buildYmlMatchesEmbed is the host-prelifted verdict for <dir>/build.yml.
// hostDeployPath is accepted for signature parity with the other migrators; this
// step touches only project files.
func migrateSingleFilename(dir, hostDeployPath string, dryRun bool, buildYmlMatchesEmbed bool) ([]string, error) {
	_ = hostDeployPath
	var changed []string
	...
	if rewriteBuildImport(rootMap, dir, dryRun, buildYmlMatchesEmbed) {
		rootChanged = true
...
func rewriteBuildImport(rootMap *yaml.Node, dir string, dryRun bool, localMatchesEmbed bool) bool {
	...
	// localMatchesEmbed is HOST-PRELIFTED (charly core ran the
	// localBuildMatchesEmbeddedVocab comparison, which needs mergeUnifiedDocs +
	// embeddedDefaults — both package-main, unreachable from this candy).
	changed := false
	var kept []*yaml.Node
...
// localBuildMatchesEmbeddedVocab moved to charly core (charly/migrate.go) — it
// needs mergeUnifiedDocs + embeddedDefaults (package-main). Core runs it as the
// single-filename host-prelift and passes the verdict in MigrateContext.
