...
// MigrateDeployEntity is the exported entry to the legacy-body → node-form
// transform, reused by charly core's per-host deploy-state writer (saveDeployState)
// so it can never drift from the unified-node migration (C13a).
func MigrateDeployEntity(name string, body *yaml.Node) *yaml.Node {
	return migrateDeployEntity(name, body)
}
