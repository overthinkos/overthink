...
// provider.go is the (inert) gRPC half of this command-only plugin. command:preempt is
...
// Invoke is unreachable for this command-only plugin: charly dispatches command:preempt by
...
func (provider) Invoke(context.Context, *pb.InvokeRequest) (*pb.InvokeReply, error) {
	return nil, fmt.Errorf("plugin-preempt: command:preempt is dispatched via the CLI (charly fork/execs this binary), not gRPC Invoke")
}
...
// Describe advertises NO gRPC capability — command:preempt is CLI-dispatched, not resolved
