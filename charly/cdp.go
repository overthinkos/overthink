...
func resolveTabWS(devtoolsURL, tabID string) (string, error) {
	...
	// doesn't resolve, preserving the original behavior for callers that
	// DO have the UUID in hand.
...
func diagnoseCDP(engine, containerName, image string, origErr error) error {
	...
	if !chromeAlive {
		...
		fmt.Fprintf(os.Stderr, "  charly sway exec %s chrome-wrapper\n", imgArg)
	} else {
		fmt.Fprintf(os.Stderr, "Hint: Chrome is running but CDP is not responding.\n")
		fmt.Fprintf(os.Stderr, "  Kill and restart:\n")
		fmt.Fprintf(os.Stderr, "  charly shell %s -c \"pkill -9 -f 'chrome.*remote-debugging'\"\n", imgArg)
		fmt.Fprintf(os.Stderr, "  charly sway exec %s chrome-wrapper\n", imgArg)
	}
