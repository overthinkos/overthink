...
//	        steps:
//	          - then: "/etc/charly-bench-marker exists"
...
var (
	...
	ErrNoRecipes = errors.New("check: no recipes configured (add a 'recipe:' map to check.yml)")
...
// Used by `charly check list-recipe`. Recipes are pure spec, so the table
// shows scenario count + description summary.
func PrintRecipe(w io.Writer, catalog map[string]*HarnessRecipe) {
	if len(catalog) == 0 {
		fmt.Fprintln(w, "No recipes configured. Add a 'recipe:' map to check.yml.")
		return
	}
