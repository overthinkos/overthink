...
// CheckRecipeCmd implements `charly check recipe <name>` — runs all scenarios
// in the named recipe ONCE against the live deployments declared via
// scenario.pod, without the AI iteration loop. Differs from `charly check run`
// (which drives an AI through plateau-bounded iterations against a score)
// in that this verb is purely a deterministic scenario checkuator: load,
// run, report.
//
...
// path RunCheckLive uses for non-dotted names). Slash- or dot-form pod
// names (e.g. `jupyter/concurrency-test`) walk the deploy tree.
type CheckRecipeCmd struct {
	Name   string `arg:"" help:"Recipe name (from check.yml recipe: map)"`
...
func (c *CheckRecipeCmd) Run() error {
	dir, err := os.Getwd()
	if err != nil {
		return err
	}
	...
	if uf == nil || len(uf.Recipe) == 0 {
		return fmt.Errorf("check recipe: no recipes configured (add a `recipe:` map to check.yml)")
	}
	...
	if err := ExpandRecipeFrom(uf, nil, c.Name, recipe); err != nil {
		return fmt.Errorf("check recipe: expanding from imports: %w", err)
	}
	scenarios := recipe.Scenario
	if len(scenarios) == 0 {
		return fmt.Errorf("check recipe %q: no scenarios", c.Name)
	}

	// Dispatch via RunCheckLive — the scenario-by-scenario probe driver.
	res, err := RunCheckLive(context.Background(), "", c.Name, scenarios, RunScoringOpts{})
	if err != nil {
		return fmt.Errorf("check recipe %q: %w", c.Name, err)
	}
...
func printRecipeText(res *CheckRunResults) {
	if res == nil {
		return
	}
	// Sort scenario results for stable output.
	sorted := make([]ScenarioCheckResult, len(res.Scenario))
	...
	for _, s := range sorted {
		...
		// Surface failed step text (StepCheckResult doesn't carry message).
		if s.Status == "fail" {
...
func printRecipeTAP(res *CheckRunResults) {
