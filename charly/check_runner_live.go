...
//  2. AI builds whatever images each pod needs and `charly deploy add`s
//     each pod by name inside harness sandbox's nested podman.
//  3. After the AI exits, this code groups scenarios by their `Pod`
//     field and runs each group's scenarios against `charly-<pod>` via
...
// RunCheckLive scores `scenarios` against the live containers
// they target via their `Pod` field. Returns a CheckRunResults shaped
...
// a separate `ContainerExecutor{ContainerName: "charly-" + pod}`. If a
...
func RunCheckLive(ctx context.Context, deployment, scoreName string, scenarios []Scenario, opts RunScoringOpts) (*CheckRunResults, error) {
	...
	// IDs used by ScenarioCheckResult must match the IDs that
	...
	for _, bucket := range buckets {
		...
		// before the scenarios and `charly deploy del` after — the check
		...
		if isEphemeralDeploy(deployRoots, pod) {
			...
			ephemeralCleanup = func(failed bool) {
				...
				fmt.Fprintf(os.Stderr, "score live: ephemeral wrap — charly deploy del %s\n", pod)
		...
		// fall back to ContainerChain (single-hop into "charly-<pod>"),
		...
		if reachableErr == nil {
			...
			// in checkrun.go (so cdp/wl/vnc/mcp dispatch routes through
			// `charly check cdp <method> <target>` correctly). Closes over
			...
			runner.TargetResolver = func(target string) (*CheckVarResolver, DeployExecutor, error) {
				...
				return &CheckVarResolver{}, exec, nil
			}
			...
			// instead of hardcoding `charly-redis`. ${PEER_ENDPOINT} (host-vantage)
	...
	if len(cyclicKeys) > 0 {
		for _, sc := range scenarios {
			...
			out.Scenario = append(out.Scenario, ScenarioCheckResult{
				ID:            idByKey[keyOf(sc)],
				Origin:        "pod:" + sc.Pod,
				Name:          sc.Name,
...
// RunCheckLive when topoSortByDeclarationOrder reports a
...
func skippedResult(sc Scenario, id, blockedBy string) ScenarioCheckResult {
...
func resolveScoringChain(roots map[string]DeploymentNode, pod string) (DeployExecutor, error) {
	if strings.Contains(pod, ".") && roots != nil {
		...
		// than a silent fall-back to a non-existent charly-foo.bar
		// container.
	...
	// shared selector instead of fabricating a non-existent charly-<pod> container
	// (mirrors check-live's local dispatch — R3).
	if roots != nil {
		if node, ok := roots[pod]; ok && classifyTarget(&node) == "local" {
			return rootExecutorForDeployNode(&node)
		}
	}
	// Flat pod or no tree — single-hop into "charly-<pod>", same shape
...
// inside RunCheckLive (which also buckets by Pod).
//
...
func synthesizeScoreBaseline(scoreName string, scenarios []Scenario) ([]ScenarioCheckResult, map[string]string, map[string]string) {
	...
	// RunCheckLive so baseline IDs == runtime IDs.
	bucketIdx := map[string]int{}
	for _, scenario := range scenarios {
		...
		for _, es := range expanded {
			...
			out = append(out, ScenarioCheckResult{
				ID:           id,
				Origin:       origin,
				Name:         es.Name,
				Tag:          append([]string(nil), es.Tag...),
...
// charly.yml entry marked ephemeral. Used by RunCheckLive to wrap
