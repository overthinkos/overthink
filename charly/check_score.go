...
// benchmark_score.go — scoring primitives for `charly check`.
//
// Exported surface:
//   - ParseCharlyTestOutput(yaml) (*CheckRunResults, error)
//       Parses the YAML emitted by `charly check box --format yaml`.
...
// description_spec.go; nothing else from the charly package. Stays standalone
...
// CheckRunResults is the structured result of one `charly check box` run,
...
type CheckRunResults struct {
	...
	Scenario []ScenarioCheckResult `yaml:"scenario,omitempty"`
	Summary  TestRunSummary       `yaml:"summary"`
}

// ScenarioCheckResult is the checkuator's verdict for a single scenario
...
type ScenarioCheckResult struct {
	ID           string           `yaml:"id"`
	...
	Step         []StepCheckResult `yaml:"step,omitempty"`
...
// StepCheckResult is the per-step detail inside a ScenarioCheckResult.
...
//
//	charly check box <tag> --format yaml
//
// into a *CheckRunResults. The parser is strict on shape (unknown
...
func deriveSummary(scenarios []ScenarioCheckResult) TestRunSummary {
...
// empty map for a nil or empty CheckRunResults.
func (r *CheckRunResults) ScenarioByID() map[string]ScenarioCheckResult {
	out := make(map[string]ScenarioCheckResult)
	if r == nil {
		return out
	}
