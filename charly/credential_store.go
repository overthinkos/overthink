...
func DefaultCredentialStore() CredentialStore {
	defaultStoreOnce.Do(func() {
		...
		default: // "auto" or ""
			...
			// 2. Fall back to config file (defaultStoreProbeErr already set)
			defaultStoreVal = &ConfigFileStore{}
		}
	})
...
//   - "keyring"     — found in the system keyring (Secret Service)
...
func ResolveCredential(envVar, service, key, defaultVal string) (value, source string) {
	...
	// If the primary store is the keyring, also check config file as fallback
...
func (c *ConfigMigrateSecretsCmd) Run() error {
	...
	if err := keyring.Probe(); err != nil {
		...
		fmt.Fprintln(os.Stderr, "  3. Keep using config file: charly config set secret_backend config")
	...
	fmt.Fprintln(os.Stderr, "Migration complete. To verify: charly config list")
