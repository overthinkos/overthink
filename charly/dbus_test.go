...
func TestParseDbusArgsMultiple(t *testing.T) {
	args := []string{"string:charly", "uint32:0", "string:", "string:title", "string:body", "int32:-1"}
	...
	if s, ok := result[0].(string); !ok || s != "charly" {
		t.Errorf("result[0] = %v, want string(charly)", result[0])
	}
