...
// subprocess exit 2→2. Underpins the `charly eval run <bed>` exit-code
...
// (port:/volume:/env:/tunnel:) never reached the per-host deploy.yml: charly eval
...
func TestBedEvalLiveRefs(t *testing.T) {
	...
	// `charly eval live` can resolve — so it gets NO dotted hop, while a pod sibling
