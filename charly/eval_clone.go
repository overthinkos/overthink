...
//
// All harness state lives under <ProjectDir>/.eval/<score>/. The
...
// in-pod run-local writes under /workspace/.eval/ (the bind-mounted
...
func HarnessDataRoot(projectDir, score string) string {
	return filepath.Join(projectDir, ".eval", score)
}
...
// ListRuns walks <projectDir>/.eval/*/runs/ across all scores
...
func ListRuns(ctx context.Context, projectDir string) ([]RunSummary, error) {
	base := filepath.Join(projectDir, ".eval")
