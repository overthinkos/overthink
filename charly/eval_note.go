...
// transient clone.
//
// Outside a run, `charly eval note read` defaults to the most recent
// run's notes file. `charly eval note append` outside a run errors —
// notes are run-scoped, ad-hoc seeding from the CLI is unsupported
// (use `charly eval note append <score> <text>` only inside an
