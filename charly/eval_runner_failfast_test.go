...
func TestScorePodTargetEntry_NilConfigFailsFastWithRemediation(t *testing.T) {
	_, err := scorePodTargetEntry(nil, "scaffolding-selftest", "eval-sandbox")
	if err == nil {
	...
	for _, want := range []string{
		...
		`pod "eval-sandbox"`,
		"charly deploy add eval-sandbox <ref> --disposable",
		"charly start eval-sandbox",
	} {
...
func TestScorePodTargetEntry_MissingEntryFailsFast(t *testing.T) {
	...
	_, err := scorePodTargetEntry(cfg, "default", "eval-sandbox")
	if err == nil {
		t.Fatal("expected an error for a missing eval-sandbox entry, got nil")
	}
...
func TestScorePodTargetEntry_PresentEntryIsReturned(t *testing.T) {
	cfg := &DeployConfig{Deploy: map[string]DeploymentNode{
		"eval-sandbox": {Disposable: disposableTrue()},
	}}
	entry, err := scorePodTargetEntry(cfg, "default", "eval-sandbox")
	if err != nil {
