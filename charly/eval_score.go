...
// ---------------------------------------------------------------------------
// `charly eval box --format yaml` parser
// ---------------------------------------------------------------------------

// EvalRunResults is the structured result of one `charly eval box` run,
...
// StepEvalResult is the per-step detail inside a ScenarioEvalResult.
...
// TestRunSummary mirrors the summary block emitted by `charly eval box
...
func ParseCharlyTestOutput(b []byte) (*EvalRunResults, error) {
	...
	var r EvalRunResults
	dec := yaml.NewDecoder(strings.NewReader(string(b)))
	dec.KnownFields(true) // strict on top-level keys
	if err := dec.Decode(&r); err != nil {
		return nil, fmt.Errorf("parse charly eval box --format yaml: %w", err)
	}
...
func deriveSummary(scenarios []ScenarioEvalResult) TestRunSummary {
...
// empty map for a nil or empty EvalRunResults.
func (r *EvalRunResults) ScenarioByID() map[string]ScenarioEvalResult {
	out := make(map[string]ScenarioEvalResult)
	if r == nil {
		return out
	}
