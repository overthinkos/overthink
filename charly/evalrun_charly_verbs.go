...
//
//	path     — CLI subcommand path after "eval", e.g. ["cdp", "eval"] or
...
// ---------------------------------------------------------------------------
//
// The mcp verb dispatches to `charly eval mcp <method> <image> …`, which uses
...
// ---------------------------------------------------------------------------
// record methods — `charly eval record <method>` drives in-container recording
...
// ---------------------------------------------------------------------------
// spice methods — `charly eval spice <method>` speaks the SPICE wire protocol
...
// ---------------------------------------------------------------------------
// libvirt methods — `charly eval libvirt <method>` uses go-libvirt RPC against
...
// ---------------------------------------------------------------------------
// k8s methods — `charly eval k8s <method>` probes a Kubernetes cluster via the
...
// ---------------------------------------------------------------------------
// appium methods — `charly eval appium <method>` drives Appium WebDriver via the
...
var appiumMethods = map[string]methodSpec{
	...
	// Tier 2 — gesture group (wl sway-style flat names → `charly eval appium gesture <op>`)
...
func posK8sRaw(c *Check) []string {
	...
	if c.JSON {
		...
		// `charly eval k8s raw` invocation. List-mode then emits the
...
// outer `charly eval libvirt guest exec` invocation.
...
func (r *Runner) runCharlyVerb(ctx context.Context, c *Check, verb, method string, allowlist map[string]methodSpec) EvalResult {
		...
		if err != nil {
			return failf(c,
				"%s: %s: validate_ai_artifacts requires the AI to have produced %q "+
					"during its iteration (e.g. via `charly eval self-evaluate`); "+
	...
	// stdout is empty — some verbs print to stderr per /charly-build:eval
