...
// EvalResults, filters by step-id glob patterns in OverIDs, and
...
func (r *Runner) runSummarize(ctx context.Context, c *Op) EvalResult {
	...
	matchedIDs := []string{}
	for stepID, evalRes := range results {
		for _, glob := range c.OverIDs {
			matched, err := filepath.Match(glob, stepID)
			if err == nil && matched {
				samples = append(samples, evalRes.Elapsed)
	...
	// Build the summary message — pasted into the EvalResult so reporters
	// surface it.
