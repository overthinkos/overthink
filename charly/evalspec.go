...
type Check struct {
	...
	// the underlying `charly eval k8s raw` invocation passes --json so the
	...
	// adb / appium-specific modifiers. See /charly-eval:adb and /charly-eval:appium.
	...
	// record-specific modifiers — record: verb wraps `charly eval record <method>`.
