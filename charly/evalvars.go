...
// EvalVarResolver holds the materials for variable expansion under `charly eval live`
// (running container present) and `charly eval box` (build-time, no container).
...
// ResolveEvalVarsRuntime builds a variable map for `charly eval live` against a
