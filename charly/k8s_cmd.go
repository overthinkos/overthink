...
func (c *K8sRawCmd) Run() error {
	...
	if c.JSON {
		...
		// `.kind` will be e.g. "NodeList", `.apiVersion` "v1",
		// `.items[]` carries each Unstructured.
...
//
// The user-scoped auto-write to ~/.config/charly/clusters/<name>.yaml is gone.
// k3s-provisioned clusters get a kind:k8s entity authored inline (via
// `charly migrate` for existing profiles, or directly in
...
// findK8sSpec looks up a K8sSpec by name from the project's charly.yml /
