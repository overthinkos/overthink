...
type LibvirtFeatures struct {
	...
	SMM    *bool          `yaml:"smm,omitempty" json:"smm,omitempty"` // required for UEFI secure boot (D17)
...
type LibvirtHyperV struct {
	Relaxed         string            `yaml:"relaxed,omitempty" json:"relaxed,omitempty"` // "on" | "off"
...
type LibvirtSpinlocks struct {
	State   string `yaml:"state,omitempty" json:"state,omitempty"` // "on" | "off"
...
type LibvirtKVM struct {
	Hidden        string `yaml:"hidden,omitempty" json:"hidden,omitempty"` // "on" | "off"
...
type LibvirtCPU struct {
	Mode       string              `yaml:"mode,omitempty" json:"mode,omitempty"`             // host-passthrough (default) | host-model | custom
	Model      string              `yaml:"model,omitempty" json:"model,omitempty"`           // required when Mode == custom
	Check      string              `yaml:"check,omitempty" json:"check,omitempty"`           // none (default) | partial | full
	Migratable string              `yaml:"migratable,omitempty" json:"migratable,omitempty"` // "on" | "off"
...
type LibvirtCPUFeature struct {
	Policy string `yaml:"policy" json:"policy"` // force | require | optional | disable | forbid
	Name   string `yaml:"name" json:"name"`
}

type LibvirtCPUCache struct {
	Mode  string `yaml:"mode,omitempty" json:"mode,omitempty"`   // emulate | passthrough | disable
	Level int    `yaml:"level,omitempty" json:"level,omitempty"` // cache level (1, 2, 3)
}

type LibvirtNUMACell struct {
	ID        int    `yaml:"id,omitempty" json:"id,omitempty"`
	CPUs      string `yaml:"cpus,omitempty" json:"cpus,omitempty"`     // "0-3"
	Memory    string `yaml:"memory,omitempty" json:"memory,omitempty"` // "2048"
	Unit      string `yaml:"unit,omitempty" json:"unit,omitempty"`     // "MiB"
...
type LibvirtClock struct {
	Offset     string         `yaml:"offset,omitempty" json:"offset,omitempty"`         // utc | localtime | timezone | variable | absolute
	Timezone   string         `yaml:"timezone,omitempty" json:"timezone,omitempty"`     // when Offset == timezone
	Adjustment string         `yaml:"adjustment,omitempty" json:"adjustment,omitempty"` // when Offset == variable
	Basis      string         `yaml:"basis,omitempty" json:"basis,omitempty"`           // when Offset == variable: "utc" | "localtime"
	Timers     []LibvirtTimer `yaml:"timers,omitempty" json:"timers,omitempty"`
}

type LibvirtTimer struct {
	Name       string `yaml:"name" json:"name"`                                 // rtc | pit | hpet | tsc | kvmclock | hypervclock | armvtimer
	Present    string `yaml:"present,omitempty" json:"present,omitempty"`       // "yes" | "no"
	Track      string `yaml:"track,omitempty" json:"track,omitempty"`           // boot | guest | wall | realtime
	TickPolicy string `yaml:"tickpolicy,omitempty" json:"tickpolicy,omitempty"` // delay | catchup | merge | discard
	Frequency  int    `yaml:"frequency,omitempty" json:"frequency,omitempty"`
	Mode       string `yaml:"mode,omitempty" json:"mode,omitempty"` // auto | native | emulate | paravirt | smpsafe
}
...
type LibvirtMemoryBacking struct {
	...
	Source       string            `yaml:"source,omitempty" json:"source,omitempty"`         // file | anonymous | memfd
	Access       string            `yaml:"access,omitempty" json:"access,omitempty"`         // shared | private
	Allocation   string            `yaml:"allocation,omitempty" json:"allocation,omitempty"` // immediate | ondemand
	Discard      *bool             `yaml:"discard,omitempty" json:"discard,omitempty"`
}

type LibvirtHugepages struct {
	Size    string `yaml:"size,omitempty" json:"size,omitempty"` // "2M" | "1G"
...
type LibvirtMemTune struct {
	HardLimit     string `yaml:"hard_limit,omitempty" json:"hard_limit,omitempty"` // e.g. "8G"
...
type LibvirtNUMAMemory struct {
	Mode      string `yaml:"mode,omitempty" json:"mode,omitempty"`           // strict | preferred | interleave | restrictive
	Nodeset   string `yaml:"nodeset,omitempty" json:"nodeset,omitempty"`     // "0-1"
	Placement string `yaml:"placement,omitempty" json:"placement,omitempty"` // static | auto
}
...
type LibvirtVCPUPin struct {
	...
	CPUSet string `yaml:"cpuset" json:"cpuset"` // "0-3" | "0,2,4"
}

type LibvirtEmulatorPin struct {
	CPUSet string `yaml:"cpuset" json:"cpuset"`
}

type LibvirtIOThreadPin struct {
	IOThread int    `yaml:"iothread" json:"iothread"`
	CPUSet   string `yaml:"cpuset" json:"cpuset"`
}
...
type LibvirtDevices struct {
	Emulator    string              `yaml:"emulator,omitempty" json:"emulator,omitempty"`     // path to qemu binary (e.g. /usr/bin/qemu-system-x86_64)
	Disks       []LibvirtDisk       `yaml:"disks,omitempty" json:"disks,omitempty"`           // additional disks beyond the root qcow2
	Interfaces  []LibvirtInterface  `yaml:"interfaces,omitempty" json:"interfaces,omitempty"` // additional NICs beyond user-mode SSH-forwarded
	Channels    []LibvirtChannel    `yaml:"channels,omitempty" json:"channels,omitempty"`     // qemu-guest-agent, spice-webdav
	...
	Graphics    []LibvirtGraphics   `yaml:"graphics,omitempty" json:"graphics,omitempty"` // vnc, spice, rdp
	...
	Hostdevs    []LibvirtHostdev    `yaml:"hostdevs,omitempty" json:"hostdevs,omitempty"`       // PCI / USB / SCSI passthrough
	Filesystems []LibvirtFilesystem `yaml:"filesystems,omitempty" json:"filesystems,omitempty"` // virtiofs / 9p host↔guest shares
...
type LibvirtDisk struct {
	Type     string            `yaml:"type,omitempty" json:"type,omitempty"`     // file | block | network | volume
	Device   string            `yaml:"device,omitempty" json:"device,omitempty"` // disk | cdrom | floppy | lun
	Source   map[string]string `yaml:"source,omitempty" json:"source,omitempty"` // {file: /path} | {dev: /dev/…} | {pool, volume} etc.
	Target   map[string]string `yaml:"target,omitempty" json:"target,omitempty"` // {dev: vda, bus: virtio}
	Driver   map[string]string `yaml:"driver,omitempty" json:"driver,omitempty"` // {name: qemu, type: qcow2, cache: none, io: native}
	Readonly *bool             `yaml:"readonly,omitempty" json:"readonly,omitempty"`
	Serial   string            `yaml:"serial,omitempty" json:"serial,omitempty"`
	WWN      string            `yaml:"wwn,omitempty" json:"wwn,omitempty"`
	Boot     int               `yaml:"boot,omitempty" json:"boot,omitempty"` // boot order
}

type LibvirtInterface struct {
	Type         string               `yaml:"type,omitempty" json:"type,omitempty"`     // user | bridge | network | direct
	Source       map[string]string    `yaml:"source,omitempty" json:"source,omitempty"` // {bridge: virbr0} | {network: default}
	Model        string               `yaml:"model,omitempty" json:"model,omitempty"`   // virtio (virtio-net-pci) | e1000 | rtl8139
	...
	PortForwards []LibvirtPortForward `yaml:"port_forwards,omitempty" json:"port_forwards,omitempty"` // for type: user
}

type LibvirtPortForward struct {
	Proto string `yaml:"proto,omitempty" json:"proto,omitempty"` // tcp | udp
	Start int    `yaml:"start" json:"start"`
	To    int    `yaml:"to,omitempty" json:"to,omitempty"`
}

type LibvirtChannel struct {
	Type   string `yaml:"type,omitempty" json:"type,omitempty"` // virtio | spicevmc | unix | pty
	Name   string `yaml:"name,omitempty" json:"name,omitempty"` // e.g. org.qemu.guest_agent.0
	Path   string `yaml:"path,omitempty" json:"path,omitempty"`
	Source string `yaml:"source,omitempty" json:"source,omitempty"`
}

type LibvirtSerial struct {
	Type   string            `yaml:"type,omitempty" json:"type,omitempty"` // pty | tcp | file | unix | dev
...
type LibvirtGraphics struct {
	Type     string                   `yaml:"type" json:"type"`                             // vnc | spice | rdp | sdl
	Port     int                      `yaml:"port,omitempty" json:"port,omitempty"`         // -1 → autoport
	AutoPort string                   `yaml:"autoport,omitempty" json:"autoport,omitempty"` // "yes" | "no"
	Listen   LibvirtGraphicsListeners `yaml:"listen,omitempty" json:"listen,omitempty"`     // scalar / map / list — see libvirt_yaml_listen.go
	Passwd   string                   `yaml:"passwd,omitempty" json:"passwd,omitempty"`
	Keymap   string                   `yaml:"keymap,omitempty" json:"keymap,omitempty"`
	GL       string                   `yaml:"gl,omitempty" json:"gl,omitempty"` // "yes" | "no" (for 3D accel)
}

type LibvirtVideo struct {
	Model   string `yaml:"model" json:"model"`                   // virtio | vga | cirrus | qxl | bochs | ramfb | none
	VRAM    int    `yaml:"vram,omitempty" json:"vram,omitempty"` // kilobytes
...
type LibvirtAudio struct {
	Type string `yaml:"type,omitempty" json:"type,omitempty"` // none | spice | oss | pulse | alsa | coreaudio
	ID   int    `yaml:"id,omitempty" json:"id,omitempty"`
}

type LibvirtSound struct {
	Model string `yaml:"model" json:"model"` // ac97 | ich6 | ich9 | usb | virtio
}

type LibvirtInput struct {
	Type string `yaml:"type" json:"type"`                   // tablet | mouse | keyboard
	Bus  string `yaml:"bus,omitempty" json:"bus,omitempty"` // ps2 | usb | virtio
}
...
type LibvirtRedirDev struct {
	Bus  string `yaml:"bus,omitempty" json:"bus,omitempty"`   // usb
	Type string `yaml:"type,omitempty" json:"type,omitempty"` // spicevmc | tcp
}

type LibvirtHostdev struct {
	Type    string            `yaml:"type" json:"type"`                           // pci | usb | scsi | mdev
	Mode    string            `yaml:"mode,omitempty" json:"mode,omitempty"`       // subsystem | capabilities
	Managed string            `yaml:"managed,omitempty" json:"managed,omitempty"` // "yes" | "no"
	Source  map[string]string `yaml:"source" json:"source"`                       // {domain, bus, slot, function} | {vendor, product} | …
	ROM     map[string]string `yaml:"rom,omitempty" json:"rom,omitempty"`
	Driver  map[string]string `yaml:"driver,omitempty" json:"driver,omitempty"`
}

type LibvirtFilesystem struct {
	Type       string            `yaml:"type,omitempty" json:"type,omitempty"`             // mount (default) | block | file | template | ram
	Driver     string            `yaml:"driver,omitempty" json:"driver,omitempty"`         // virtiofs | 9p | path
	AccessMode string            `yaml:"accessmode,omitempty" json:"accessmode,omitempty"` // passthrough | mapped | squash
	Source     string            `yaml:"source" json:"source"`                             // host path
	Target     string            `yaml:"target" json:"target"`                             // guest mount tag
	Readonly   *bool             `yaml:"readonly,omitempty" json:"readonly,omitempty"`
	Binary     map[string]string `yaml:"binary,omitempty" json:"binary,omitempty"` // virtiofsd knobs
}

type LibvirtRNG struct {
	Model   string            `yaml:"model,omitempty" json:"model,omitempty"`     // virtio (default)
	Backend string            `yaml:"backend,omitempty" json:"backend,omitempty"` // /dev/urandom | /dev/random | builtin | egd
	Rate    map[string]string `yaml:"rate,omitempty" json:"rate,omitempty"`       // {period, bytes}
}

type LibvirtTPM struct {
	Model   string            `yaml:"model,omitempty" json:"model,omitempty"`     // tpm-tis | tpm-crb | tpm-spapr
	Backend map[string]string `yaml:"backend,omitempty" json:"backend,omitempty"` // {type: emulator, version: "2.0"} | {type: passthrough, path: ...}
}

type LibvirtWatchdog struct {
	Model  string `yaml:"model" json:"model"`                       // i6300esb | ib700 | diag288 | itco
	Action string `yaml:"action,omitempty" json:"action,omitempty"` // reset | shutdown | poweroff | pause | none | dump | inject-nmi
}

type LibvirtMemBalloon struct {
	Model       string         `yaml:"model" json:"model"` // virtio | none
	Autodeflate string         `yaml:"autodeflate,omitempty" json:"autodeflate,omitempty"`
	Stats       map[string]int `yaml:"stats,omitempty" json:"stats,omitempty"` // {period: 5}
}

type LibvirtShmem struct {
	Name   string            `yaml:"name" json:"name"`
	Role   string            `yaml:"role,omitempty" json:"role,omitempty"`
	Model  map[string]string `yaml:"model,omitempty" json:"model,omitempty"` // {type: ivshmem-plain | ivshmem-doorbell}
	Size   string            `yaml:"size,omitempty" json:"size,omitempty"`
	Server map[string]string `yaml:"server,omitempty" json:"server,omitempty"`
}

type LibvirtIOMMU struct {
	Model  string            `yaml:"model" json:"model"` // intel | smmuv3 | virtio
	Driver map[string]string `yaml:"driver,omitempty" json:"driver,omitempty"`
}

type LibvirtVsock struct {
	Model string            `yaml:"model,omitempty" json:"model,omitempty"` // virtio
	CID   map[string]string `yaml:"cid,omitempty" json:"cid,omitempty"`     // {auto: "yes"} or {address: "3"}
}

type LibvirtPanic struct {
	Model   string            `yaml:"model,omitempty" json:"model,omitempty"` // isa | pseries | hyperv | s390 | pvpanic
	Address map[string]string `yaml:"address,omitempty" json:"address,omitempty"`
}

type LibvirtSmartcard struct {
	Mode string `yaml:"mode,omitempty" json:"mode,omitempty"` // host | host-certificates | passthrough
	Type string `yaml:"type,omitempty" json:"type,omitempty"`
}

type LibvirtHub struct {
	Type string `yaml:"type" json:"type"` // usb
}

// LibvirtSecLabel maps to <seclabel>. Commonly used with SELinux.
type LibvirtSecLabel struct {
	Type       string `yaml:"type,omitempty" json:"type,omitempty"`       // dynamic | static | none
	Model      string `yaml:"model,omitempty" json:"model,omitempty"`     // selinux | dac | apparmor
	Relabel    string `yaml:"relabel,omitempty" json:"relabel,omitempty"` // "yes" | "no"
...
type LibvirtLaunchSecurity struct {
	Type            string `yaml:"type,omitempty" json:"type,omitempty"` // sev | sev-es | sev-snp | tdx
	...
	Policy          string `yaml:"policy,omitempty" json:"policy,omitempty"` // hex string
	DhCert          string `yaml:"dh_cert,omitempty" json:"dh_cert,omitempty"`
	Session         string `yaml:"session,omitempty" json:"session,omitempty"`
	KernelHashes    string `yaml:"kernel_hashes,omitempty" json:"kernel_hashes,omitempty"` // "yes" | "no"
}
...
type LibvirtSysInfo struct {
	Type       string              `yaml:"type,omitempty" json:"type,omitempty"` // smbios (default) | fwcfg
	...
	OEMStrings []string            `yaml:"oem_strings,omitempty" json:"oem_strings,omitempty"` // renderer populates this for SMBIOS SSH-key injection
}
