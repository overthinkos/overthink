...
func TestBuildDeployPlanLocalPkgOrdering(t *testing.T) {
	l := &Candy{
		...
		plan: []Step{
			{Run: "build", Op: Op{Command: "echo install charly", RunAs: "root"}},
		},
	}
...
func TestLocalPkgMapRejectsScalar(t *testing.T) {
	var m LocalPkgMap
	if err := yaml.Unmarshal([]byte("pkg/arch\n"), &m); err == nil || !strings.Contains(err.Error(), "charly migrate") {
		t.Errorf("scalar localpkg should be rejected with an `charly migrate` hint, got %v", err)
	}
	m = nil
