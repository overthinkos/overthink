...
// files.
func MigrateOvCachyos(dir string, dryRun bool) ([]string, error) {
	var changed []string
	...
	for _, p := range projectFiles {
		modified, err := rewriteOvCachyosFile(p, dryRun)
		if err != nil {
			return changed, err
		}
		if modified {
	...
	if home, err := os.UserHomeDir(); err == nil {
		...
		if modified, err := rewriteOvCachyosFile(userFile, dryRun); err == nil && modified {
...
// rewriteOvCachyosFile applies the consolidated rename to a single
// YAML file. Returns (modified, error). Missing files are NOT errors
// — the migration is opportunistic per file.
func rewriteOvCachyosFile(path string, dryRun bool) (bool, error) {
	...
	updated := applyOvCachyosRewrites(string(data))
...
// applyOvCachyosRewrites returns src with all rename rewrites
// applied. Comment text mentioning qc / cachyos-dx is also touched
// up so surrounding prose stays in sync. The rewrite is narrow:
// only references in the deployment-key idiom + a small set of
// documented comment idioms are touched.
func applyOvCachyosRewrites(src string) string {
	out := qcDeployKeyPattern.ReplaceAllString(src, "${1}ov-cachyos:")
	out = cachyosDxDeployKeyPattern.ReplaceAllString(out, "${1}ov-cachyos:")
	out = cachyosDxCrossRefPattern.ReplaceAllString(out, "${1}local: ov-cachyos")
	...
	commentRewrites := []struct {
		...
		// qc → ov-cachyos
		{"# qc — this CachyOS workstation", "# ov-cachyos — this CachyOS workstation"},
		{"`charly rebuild qc`", "`charly rebuild ov-cachyos`"},
		{"'charly rebuild qc'", "'charly rebuild ov-cachyos'"},
		{"charly rebuild qc", "charly rebuild ov-cachyos"},
		{"charly deploy add qc", "charly deploy add ov-cachyos"},
		{"qc CachyOS host", "ov-cachyos CachyOS host"},
		{"# qc host", "# ov-cachyos host"},
		{"`qc` deployment", "`ov-cachyos` deployment"},
		{"the `qc` deployment", "the `ov-cachyos` deployment"},
		{"qc host", "ov-cachyos host"},
		// cachyos-dx → ov-cachyos
		{"# cachyos-dx — this CachyOS workstation", "# ov-cachyos — this CachyOS workstation"},
		{"`charly rebuild cachyos-dx`", "`charly rebuild ov-cachyos`"},
		{"'charly rebuild cachyos-dx'", "'charly rebuild ov-cachyos'"},
		{"charly rebuild cachyos-dx", "charly rebuild ov-cachyos"},
		{"charly deploy add cachyos-dx", "charly deploy add ov-cachyos"},
		{"charly test cachyos-dx", "charly eval live ov-cachyos"},
		...
		{"the `cachyos-dx` deployment", "the `ov-cachyos` deployment"},
		{"cachyos-dx host", "ov-cachyos host"},
		{"Apply cachyos-dx to this workstation", "Apply ov-cachyos to this workstation"},
	}
