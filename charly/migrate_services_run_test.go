...
// gated by the CH_RUN_MIGRATION env var so normal `go test` runs don't
// mutate the repo. Invoke with:
//
//	CH_RUN_MIGRATION=1 go test -run '^TestRunMigrateServices$' ./...
//
...
func TestRunMigrateServices(t *testing.T) {
	if os.Getenv("CH_RUN_MIGRATION") != "1" {
		t.Skip("skipping migration (set CH_RUN_MIGRATION=1 to run)")
	}
	dir := os.Getenv("CH_MIGRATION_DIR")
	if dir == "" {
		dir = "../layers"
	}
