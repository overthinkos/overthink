...
func TestImportList_Unmarshal(t *testing.T) {
	root := t.TempDir()
	writeFixture(t, root, "charly.yml", `version: 2026.174.0500
import:
  - build.yml
  - sub: ./sub.yml
`)
  ...
  build: [rpm]
`)
	writeFixture(t, root, "sub.yml", `version: 2026.174.0500
widget:
  candy:
...
app:
  ...
  app-distro:
    distro: [fedora]
`)
	writeFixture(t, root, "sub/charly.yml", `version: 2026.174.0500
import:
  - up: ../
widget:
  candy:
