...
func parseNode(name string, m *yaml.Node, asChild bool) (*genericNode, error) {
	...
	if gn.discClass == "entity" {
		for _, c := range childPairs {
			...
			// (a pod/vm/k8s/local/android/host/group). A non-deployable kind
			...
			if child.discClass == "entity" && !resourceKindSet[gn.disc] {
				return nil, fmt.Errorf("node %q (kind %q): sub-entity child %q (kind %q) is not allowed — only deployable kinds (pod/vm/k8s/local/android/host/group) nest sub-entities", name, gn.disc, child.name, child.disc)
			}
