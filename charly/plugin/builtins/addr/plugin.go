// Package addr is the BUILT-IN `addr` plugin unit: its self-contained CUE schema
...
// registration live in package main (plugin_addr.go) — the in-proc analogue of an
// external plugin's main.go. The `addr` verb (host-side TCP dial / in-container `nc -z`
// reachability probe) was formerly a base #Op verb; it now lives here as a dedicated
// plugin unit (mirrors process/port/dns — its provider is a CheckVerbProvider, keeping
// the live *Runner the dial needs).
package addr
...
func Schema() string {
	...
	if err != nil {
		...
		panic("addr: read embedded schema: " + err.Error())
	}
	return body
}

// InputDefs maps the provided capability ("verb:addr") to the CUE def that validates
// its plugin_input. The host carries this on the unit's PluginSchema, so
// validateAuthoredPluginInput finds #AddrInput for the addr verb.
