// Package matching is the BUILT-IN `matching` plugin unit: its self-contained CUE
...
// unit registration live in package main (plugin_matching.go) — the in-proc analogue
// of an external plugin's main.go. The `matching` verb (pure in-process value matching)
// was formerly a base #Op verb; it now lives here as a dedicated plugin unit, the
// in-proc analogue of an external plugin's main.go (mirrors exampleprobe).
package matching
...
func Schema() string {
	...
	if err != nil {
		...
		panic("matching: read embedded schema: " + err.Error())
	}
	return body
}

// InputDefs maps the provided capability ("verb:matching") to the CUE def that
// validates its plugin_input. The host carries this on the unit's PluginSchema, so
// validateAuthoredPluginInput finds #MatchingInput for the matching verb.
var InputDefs = map[string]string{"verb:matching": "#MatchingInput"}
