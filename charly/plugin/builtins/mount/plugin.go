// Package mount is the BUILT-IN `mount` plugin unit: its self-contained CUE schema
...
// registration live in package main (plugin_mount.go) — the in-proc analogue of an
// external plugin's main.go. The `mount` verb (findmnt probe + mount) was formerly a
// base #Op verb; it now lives here as a dedicated plugin unit.
//
// It is a STATE-PROVISION verb — its provider is BOTH a CheckVerbProvider (RunVerb keeps
// the live *Runner the findmnt probe needs) AND a ProvisionActor (RenderProvisionScript
// renders the mount at install emit + runtime act).
package mount
...
func Schema() string {
	...
	if err != nil {
		...
		panic("mount: read embedded schema: " + err.Error())
	}
	return body
}

// InputDefs maps the provided capability ("verb:mount") to the CUE def that validates
// its plugin_input. The host carries this on the unit's PluginSchema, so
// validateAuthoredPluginInput finds #MountInput for the mount verb.
