// Package port is the BUILT-IN `port` plugin unit: its self-contained CUE schema
...
// registration live in package main (plugin_port.go) — the in-proc analogue of an
// external plugin's main.go. The `port` verb (in-container listening probe / host-side
// reachability dial) was formerly a base #Op verb; it now lives here as a dedicated
// plugin unit (mirrors process/examplerunverb — its provider is a CheckVerbProvider,
// keeping the live *Runner the probe + host dial need).
package port
...
func Schema() string {
	...
	if err != nil {
		...
		panic("port: read embedded schema: " + err.Error())
	}
	return body
}

// InputDefs maps the provided capability ("verb:port") to the CUE def that validates
// its plugin_input. The host carries this on the unit's PluginSchema, so
// validateAuthoredPluginInput finds #PortInput for the port verb.
