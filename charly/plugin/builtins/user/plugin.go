// Package user is the BUILT-IN `user` plugin unit: its self-contained CUE schema
...
// registration live in package main (plugin_user.go) — the in-proc analogue of an
// external plugin's main.go. The `user` verb (getent-passwd probe + useradd) was
...
// the live *Runner the getent-passwd probe needs) AND a ProvisionActor
// (RenderProvisionScript renders the useradd at install emit + runtime act).
package user
...
func Schema() string {
	...
	if err != nil {
		...
		panic("user: read embedded schema: " + err.Error())
	}
	return body
}

// InputDefs maps the provided capability ("verb:user") to the CUE def that validates its
// plugin_input. The host carries this on the unit's PluginSchema, so
// validateAuthoredPluginInput finds #UserInput for the user verb.
