...
// validation/migration, so early-running capabilities (migrate, egress) can themselves be plugins
// loaded at the right time. This is the SINGLE SOURCE for the phase vocabulary (R3): charly's
// package main aliases these, and a plugin's Describe declares its phase against them.
const (
	PhaseBootstrap = "bootstrap" // before config validation/migration — the kernel bootstrap set (migrate, egress). Compiled-in only (no validated config exists yet to discover an out-of-process source).
