...
// localTarget is the `target: local` deploy-target IR provider, extracted into its
// OWN file as the externalizable dedicated-provider pattern (Phase 3). A deploy
// target derives its word from cross-refs (`target:` is not user-authored), so it
// carries NO authored plugin_input and NO CUE schema — it therefore does not fit the
...
// providerRegistry.ResolveDeploy. Its UnifiedDeployTarget construction is unchanged
// (behavior-preserving).
...
var _ = registerDedicatedBuiltin(localTarget{})
