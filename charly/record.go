...
func (c *RecordStartCmd) Run() error {
	...
	if err := venueRunSilent(venue.Exec, "mkdir -p /tmp/charly-recordings"); err != nil {
	...
	modeFile := "/tmp/charly-recordings/" + c.Name + ".mode"
...
func (c *RecordCmdCmd) Run() error {
	...
	sendVenueNotification(venue.Exec,
		fmt.Sprintf("charly: sent to %s", session), c.Command)
...
func recordingFilePath(name, mode string) string {
	...
	return "/tmp/charly-recordings/" + name + ext
}
...
func readRecordingMode(ex DeployExecutor, name string) string {
	modeFile := "/tmp/charly-recordings/" + name + ".mode"
