...
var builtinProviderInstances = []Provider{
	// verbs (ClassVerb) — command/service/http/interface/addr/unix_group/user/kernel-param/
	// mount are NOT here: each is a dedicated plugin UNIT (plugin_verb_command.go /
	// plugin_verb_service.go / plugin_http.go / plugin_interface.go / plugin_addr.go /
	// plugin_unix_group.go / plugin_user.go / plugin_kernel_param.go / plugin_mount.go) that
	// self-registers via RegisterBuiltinPluginUnit, absent from both this slice and the
	// providers: manifest (the goss-verb→plugin + the state-provision-verb→plugin
	// extractions, like process/port/dns; `command` is the install-task-act member and
	// `service` the typed-step-outlier member of that set).
	fileVerb{}, packageVerb{},
