...
var builtinProviderInstances = []Provider{
	// verbs (ClassVerb) — command/http/interface/addr/unix_group/user/kernel-param/mount
	// are NOT here: each is a dedicated plugin UNIT (plugin_verb_command.go / plugin_http.go /
	...
	// RegisterBuiltinPluginUnit, absent from both this slice and the providers: manifest
	// (the goss-verb→plugin + the state-provision-verb→plugin extractions, like
	// process/port/dns; `command` is the install-task-act member of that set).
	fileVerb{}, packageVerb{}, serviceVerb{},
