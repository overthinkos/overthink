...
// points at `charly migrate`, which strips them in place.
func validateNoKdbxResiduals(data []byte, cfg *RuntimeConfig) error {
	if cfg.SecretBackend == "kdbx" {
		return fmt.Errorf("secret_backend: kdbx — the direct KeePass .kdbx backend was removed; run `charly migrate` (KeePassXC still works via Secret Service)")
	}
	...
	for _, k := range []string{"secrets_kdbx_path", "secrets_kdbx_key_file", "kdbx_cache", "kdbx_cache_timeout"} {
		if _, ok := raw[k]; ok {
			return fmt.Errorf("legacy key %q — the direct KeePass .kdbx backend was removed; run `charly migrate`", k)
		}
	}
	return nil
}
