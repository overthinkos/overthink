...
// TestVmUnifiedTarget_vmDomainName verifies the "charly-<entity>" prefix.
func TestVmUnifiedTarget_vmDomainName(t *testing.T) {
	target := &VmUnifiedTarget{NodeName: "arch-vm"}
	if got := target.vmDomainName(); got != "charly-arch-vm" {
		t.Errorf("vmDomainName = %q, want charly-arch-vm", got)
	}
	target2 := &VmUnifiedTarget{
		NodeName:       "arch-vm",
		VmDeployTarget: &VmDeployTarget{VMName: "arch"},
	}
	if got := target2.vmDomainName(); got != "charly-arch" {
		t.Errorf("vmDomainName(with VMName) = %q, want charly-arch", got)
	}
}
...
func TestVmUnifiedTarget_Rebuild_DryRun(t *testing.T) {
	...
	if strings.Contains(out, "dry-run: charly vm build") {
