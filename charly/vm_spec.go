...
type VmSpec struct {
	...
	Source VmSource `yaml:"source" json:"source"`

	// --- Hardware (both source kinds) ---

	DiskSize string `yaml:"disk_size,omitempty" json:"disk_size,omitempty"` // e.g. "20G", "10 GiB"
	Ram      string `yaml:"ram,omitempty" json:"ram,omitempty"`             // e.g. "4G", "8192M"
	Cpus     int    `yaml:"cpu,omitempty" json:"cpu,omitempty"`             // e.g. 2 (YAML key `cpu` — native VmSpec key, singular per the field-singular cutover; rendered to libvirt <vcpu>)
	Machine  string `yaml:"machine,omitempty" json:"machine,omitempty"`     // q35 | virt (arm64) | i440fx — default: host-native
	Firmware string `yaml:"firmware,omitempty" json:"firmware,omitempty"`   // bios | uefi-insecure | uefi-secure — default: bios
	...
	Snapshots []VmSnapshotDecl `yaml:"snapshot,omitempty" json:"snapshot,omitempty"`
}
...
// read from the DeploymentNode(s) that reference it via `vm:`, through
// DeploymentNode.IsDisposable() (deploy.go). `charly update` does NOT gate on
...
type VmChecksum struct {
	Type  string `yaml:"type,omitempty" json:"type,omitempty"`   // sha256 (only supported algorithm for now)
	Value string `yaml:"value,omitempty" json:"value,omitempty"` // hex digest; empty → auto-fetch <url>.SHA256 sidecar
}
