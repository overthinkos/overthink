...
func (c *WlOverlayListCmd) Run() error {
	...
	return execWlCmd(venue.Exec, "charly-overlay list")
}
...
func (c *WlOverlayStatusCmd) Run() error {
	...
	return execWlCmd(venue.Exec, "charly-overlay status")
}

// buildOverlayShowArgs constructs the charly-overlay show command string from flags.
func buildOverlayShowArgs(c *WlOverlayShowCmd) string {
	var parts []string
	parts = append(parts, "charly-overlay", "show")
...
func ensureOverlayDaemon(ex DeployExecutor) error {
	...
	_ = execWlCmdSilent(ex, "rm -f /tmp/charly-overlay.sock")
