...
func TestOverlayDaemonSession(t *testing.T) {
	if overlayDaemonSession != "charly-overlay-daemon" {
		t.Errorf("overlayDaemonSession = %q, want %q", overlayDaemonSession, "charly-overlay-daemon")
	}
}
...
func TestBuildOverlayShowArgs(t *testing.T) {
	...
	if !strings.HasPrefix(args, "charly-overlay show") {
