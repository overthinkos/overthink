# layers/ — signpost (not the rule-set)

You are in the **layer** definitions (`layers/<name>/layer.yml` + supporting
config files: `pixi.toml`, `package.json`, `Cargo.toml`, service files, …).

**Load these skills FIRST (R0):**

- `/ov-image:layer` — the authoritative `layer.yml` schema: the `task:` verb
  catalog, `vars:` substitution, the unified `service:` schema, package
  sections, `eval:` checks, and the mandatory `version:` field.
- `/ov-image:image` — when composing layers into an image.
- `/ov-eval:eval` — authoring the `eval:` declarative checks a layer ships.

The `layer-validator` agent is a fast pre-edit sanity gate; `ov image validate`
is the authoritative checker. Use the `ov layer …` editor verbs (comment-
