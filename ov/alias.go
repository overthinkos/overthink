...
func CollectImageAlias(cfg *Config, layers map[string]*Layer, imageName string) ([]CollectedAlias, error) {
	...
	if !ok {
		return nil, fmt.Errorf("image %q not found in overthink.yml", imageName)
	}
...
type AliasAddCmd struct {
	...
	Image   string `arg:"" help:"Image name from overthink.yml"`
...
func (c *AliasAddCmd) Run() error {
	...
	// "ov box pull" recommendation via ErrImageNotLocal.
