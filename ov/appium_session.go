...
// multi-step Appium tests share one WebDriver session across separate `ov
// eval appium …` invocations. Each session-create writes a JSON file at
// ~/.cache/ov/appium/sessions/<image>[_<instance>].json; later
...
// appiumSessionsDir returns ~/.cache/ov/appium/sessions, creating it on
...
func appiumSessionsDir() (string, error) {
	...
	dir := filepath.Join(base, "ov", "appium", "sessions")
