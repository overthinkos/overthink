...
// parallelism flags. OV_BUILD_JOBS → Jobs was missing before this cutover
// (doc/code drift the build SKILL documented but the tag lacked); OV_PODMAN_JOBS
// → PodmanJobs already existed. Both are asserted here so the bindings can't
// silently regress.
func TestBuildCmd_JobsEnvBindings(t *testing.T) {
	t.Setenv("OV_BUILD_JOBS", "6")
	t.Setenv("OV_PODMAN_JOBS", "9")
	...
	if cli.Build.Jobs != 6 {
		t.Errorf("Jobs from OV_BUILD_JOBS = %d, want 6", cli.Build.Jobs)
	}
	if cli.Build.PodmanJobs != 9 {
		t.Errorf("PodmanJobs from OV_PODMAN_JOBS = %d, want 9", cli.Build.PodmanJobs)
	}
}
