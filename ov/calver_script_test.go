...
// binary built from one commit reports the IDENTICAL `ov version` — a dirty
// working-tree `task build:ov`, the clean git+file:// makepkg clone, an AUR
...
// (the pacman-pkgver vs `ov version` split) and a stale binary could falsely sort
...
func TestCalverScriptDeterministic(t *testing.T) {
	...
	// exactly the shape of a dev `task build:ov` over edited ov/*.go that the old
