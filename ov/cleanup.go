...
//      `/tmp/ov-*` for known patterns and remove anything not held by
...
// running ov processes (e.g. `ov mcp serve`).
...
// SIGKILL'd ov are caught by the next invocation's SweepStaleTemps.
...
// (`os.MkdirTemp`). Patterns that SHOULD persist across ov invocations
// (e.g. `ov-tunnel-*.sock` for long-lived SSH forwards declared in
// ssh_tunnel.go) are NOT in this list.
var sweepablePatterns = []string{
	"ov-merge-",              // merge.go saveAndLoad + saveImageToDaemon
	...
	for _, prefix := range sweepablePatterns {
		...
		for _, p := range matches {
			...
			if time.Since(info.ModTime()) < sweepSafetyFloor {
				continue // too recent — concurrent ov may still need it
			}
			if heldPaths[p] {
