...
func TestSweep_OldNotHeld_Removed(t *testing.T) {
	resetCleanupState(t)
	p := makeStaleTemp(t, "ov-merge-", "fakestale-9911-test.tar", 10*time.Minute)
	SweepStaleTemps()
	if _, err := os.Stat(p); !os.IsNotExist(err) {
...
func TestSweep_RecentNotHeld_Kept(t *testing.T) {
	...
	p := makeStaleTemp(t, "ov-merge-", "fresh-9912-test.tar", 1*time.Minute)
	SweepStaleTemps()
	if _, err := os.Stat(p); err != nil {
...
func TestSweep_HeldByCurrentProcess_Kept(t *testing.T) {
	resetCleanupState(t)
	p := makeStaleTemp(t, "ov-merge-", "held-9913-test.tar", 10*time.Minute)
...
func TestSweep_NonOvFile_Untouched(t *testing.T) {
	...
	SweepStaleTemps()
	if _, err := os.Stat(p); err != nil {
		t.Errorf("expected non-ov file kept, stat = %v", err)
	}
}

func TestSweep_TunnelSocketIgnored(t *testing.T) {
	resetCleanupState(t)
	// The ov-tunnel- prefix is intentionally NOT in sweepablePatterns —
	// SSH-tunnel sockets are session-scoped persistent.
	p := makeStaleTemp(t, "ov-tunnel-", "9915-test.sock", 10*time.Minute)
	SweepStaleTemps()
	if _, err := os.Stat(p); err != nil {
