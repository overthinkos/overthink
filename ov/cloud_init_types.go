...
// `ov vm build` / `ov deploy add vm:…` time by RenderCloudInit.
//
// No raw user_data YAML string — users declare intent here, ov produces
...
type VmCloudInit struct {
	...
	Users []VmCloudInitUser `yaml:"users,omitempty"`
...
type VmCloudInitFile struct {
	Path     string `yaml:"path"`               // absolute path inside the guest
	Content  string `yaml:"content"`            // file body (plain text)
	Owner    string `yaml:"owner,omitempty"`    // e.g. "root:root" (default)
	Perms    string `yaml:"perms,omitempty"`    // e.g. "0644" (default)
	Encoding string `yaml:"encoding,omitempty"` // "" | "b64" | "gz" | "gz+b64"
}
...
type VmCloudInitMirrors struct {
	APT    []string `yaml:"apt,omitempty"`    // Ubuntu/Debian: rewrites /etc/apt/sources.list
	DNF    []string `yaml:"dnf,omitempty"`    // Fedora: writes /etc/yum.repos.d/custom.repo
	Pacman []string `yaml:"pacman,omitempty"` // Arch: rewrites /etc/pacman.d/mirrorlist
}

// VmOvInstall controls how the ov binary lands in the guest.
...
type VmOvInstall struct {
	// Strategy is one of:
	//   auto  — scp the local ov binary (os.Executable()) into the
	...
	//   skip  — user manages ov install themselves; VmDeployTarget
