...
// DeployFromImageCmd implements `ov deploy from-image <ref> [name]` — a
// SOURCE-LESS deploy driven entirely by an image's baked org.overthinkos.* OCI
// labels, with NO overthink.yml project. Two targets:
//
...
func (c *DeployFromImageCmd) Run() error {
	...
	// quadlet, and daemon-reloads — all with no overthink.yml.
	...
	// short-name re-resolution (as `ov start` does) would resolve the wrong
	// image.
