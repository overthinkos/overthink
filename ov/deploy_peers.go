...
// up/torn down by the SAME `ov config`/`ov start`/`ov remove` verbs the deploy
...
func foldPeers(uf *UnifiedFile) error {
	...
	for _, p := range pending {
		...
		// teardown/rebuild (e.g. a kind:eval bed's ov update) is authorized to
...
// `port:` actually publishes — `ov config` otherwise sources ports from image
// labels behind an operator -p), then `ov config <peer>` + `ov start <peer>`,
// then wait for readiness. A non-pod peer (target: vm/local) is registered via
// `ov deploy add <peer>`. The SAME helper serves the kind:eval bed runner and
...
// pod peers are reversed via `ov deploy del`. Never fails the owner's teardown.
