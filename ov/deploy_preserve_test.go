...
// through the ACTUAL `ov update <vm>` path: VmUnifiedTarget.Rebuild shells
// `ov vm destroy` (removeVmDeployEntry) then `ov vm create` (saveVmDeployState).
...
func TestOvUpdatePreservesPerHostDeployFields(t *testing.T) {
	...
	cfgDir := filepath.Join(dir, "ov")
	if err := os.MkdirAll(cfgDir, 0o755); err != nil {
		t.Fatal(err)
	}
	// Per-host overlay keyed as `ov vm destroy`/`ov vm create` key it (vm:<name>):
...
`
	...
	// `ov update <vm>` == destroy (removeVmDeployEntry) THEN create
	...
	if !ok {
		t.Fatal("vm:cachyos-gpu entry vanished after ov update destroy→create")
	}
	if node.Preemptible == nil || len(node.Preemptible.Holds) != 1 || node.Preemptible.Holds[0] != "nvidia-gpu" {
		t.Errorf("ov update DROPPED preemptible: got %+v", node.Preemptible)
	}
	if len(node.Env) != 1 || node.Env[0] != "EDITOR=nvim" {
		t.Errorf("ov update DROPPED env: got %+v", node.Env)
	}
	if node.Tunnel == nil {
		t.Errorf("ov update DROPPED tunnel")
	}
...
func TestGatherDeployNodesPerHostWins(t *testing.T) {
	...
	projYml := `version: 2026.144.1443
deploy:
  cachyos-gpu:
    target: vm
    vm: cachyos-gpu
`
	if err := os.WriteFile(filepath.Join(proj, "overthink.yml"), []byte(projYml), 0o644); err != nil {
		t.Fatal(err)
	}
...
deploy:
  cachyos-gpu:
    ...
    preemptible:
      holds: [nvidia-gpu]
`
	if err := os.WriteFile(filepath.Join(cfg, "ov", "deploy.yml"), []byte(hostYml), 0o600); err != nil {
		t.Fatal(err)
	}
	t.Chdir(proj)
