...
func TestResolveDeployRefLocalImage(t *testing.T) {
	...
	// shape with version: 2026.156.1041 and the singular box: kind map.
	if err := os.WriteFile(filepath.Join(dir, "overthink.yml"), []byte(`
version: 2026.156.1041
box:
  myimg:
    base: fedora
`), 0644); err != nil {
...
`), 0644); err != nil {
	...
	_ = os.WriteFile(filepath.Join(dir, "overthink.yml"), []byte("version: 2026.156.1041\nimage: {}\n"), 0644)
...
func TestResolveDeployRefRemoteBareRepo(t *testing.T) {
	...
	// (points at the project's overthink.yml).
