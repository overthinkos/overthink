...
// emitted as the org.overthinkos.version OCI label:
//
...
//  4. a HARD ERROR pointing at `ov migrate` — there is NO build-timestamp
...
func (g *Generator) computeEffectiveVersions() error {
	...
	compute = func(name string) (string, error) {
		...
		return "", fmt.Errorf("image %q resolves no version: a layerless image on an external base needs a dedicated `version:`. Run: ov migrate", name)
	}
