...
// `ov eval` surface, and parses back a structured pass/fail verdict.
//
...
// only by `ov eval feature run <deployment>` (a live deployment the agent can
// reach); `ov box feature run` (disposable, no stable target) leaves it nil so
// prose steps stay advisory-skip, and the harness loop / `ov eval recipe`
// never set it.
...
// `ov eval feature run --timeout` flag nor the AI entry's own `timeout:` is
...
func buildGraderPrompt(req GraderRequest, target, instance string) string {
	...
	b.WriteString("  ov cmd " + target + " '<shell>'            # run a shell command inside the deployment\n")
	b.WriteString("  ov eval mcp list-tools " + target + "      # probe an MCP server it advertises\n")
	b.WriteString("  ov eval cdp status " + target + "          # Chrome DevTools (if it runs a browser)\n")
	b.WriteString("  ov eval wl screenshot " + target + " --artifact /tmp/s.png   # desktop screenshot\n")
	b.WriteString("  ov status " + target + "                   # deployment status\n")
...
func RunAIOnce(ctx context.Context, ai *AIConfig, prompt string, timeout time.Duration) (string, string, error) {
	...
	if ai.PromptVia == "file" {
		f, err := os.CreateTemp("", "ov-grader-prompt-*.md")
		if err != nil {
