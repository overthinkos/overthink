...
// runners: `ov box feature run <image>` and `ov eval feature run <deployment>`.
//
...
// tests — the RUN half of the `ov feature {list,pending,validate}` family
...
// `ov eval box` / `ov eval live` (R3); the only new behaviour is surfacing
...
//   - `ov eval feature run <name>`     — DEPLOY scope. Against a running
...
// ---------------------------------------------------------------------------
// ov box feature run <image>  (BUILD scope)
// ---------------------------------------------------------------------------

// BoxFeatureCmd groups `ov box feature run` (and room for future build-scope
...
type BoxFeatureCmd struct {
	Run BoxFeatureRunCmd `cmd:"" help:"Run an image's baked Gherkin scenarios against a disposable container (build scope; prose-only steps need a live deployment — see ov eval feature run)"`
}

// BoxFeatureRunCmd: `ov box feature run <image>`. Build-scope acceptance —
// the image's baked scenarios run against a disposable container. Image refs
// resolve against local container storage (never overthink.yml), same as
// `ov eval box`.
...
// ---------------------------------------------------------------------------
// ov eval feature run <deployment>  (DEPLOY scope)
// ---------------------------------------------------------------------------

// EvalFeatureCmd groups `ov eval feature run` under the live-eval hierarchy.
...
// EvalFeatureRunCmd: `ov eval feature run <deployment>`. Deploy-scope
...
func (c *EvalFeatureRunCmd) Run() error {
	...
	// metadata path `ov eval live` uses (R3).
	...
	// container IP, env), same as `ov eval live`.
	var deployOverlay *DeploymentNode
	if dc := loadDeployConfigForRead("ov eval feature run"); dc != nil {
