...
// findOvForEval returns the path to the ov binary the harness
...
func renderRunnerInvocation(opts HarnessOpts, substCtx *SubstContext, promptText, iterDir string) ([]string, map[string]string) {
	...
	env["OV_EVAL_RUN_ID"] = substCtx.RunID
	env["OV_EVAL_ITERATION"] = fmt.Sprintf("%d", substCtx.Iteration)
	env["OV_EVAL_SCORE"] = substCtx.ScoreName
	env["OV_EVAL_AI"] = substCtx.AIName
	...
	if opts.Score != nil && opts.Score.NotesEnabled() {
		...
		env["OV_EVAL_NOTES_FILE"] = NotePathForRun(harnessRoot, substCtx.RunID)
	}
	return argv, env
}
...
// ResolveLastTestTag reads OV_EVAL_RUN_ID + OV_EVAL_ITERATION
...
func ResolveLastTestTag(targetImage string, stdout *os.File) error {
	runID := os.Getenv("OV_EVAL_RUN_ID")
	if runID == "" {
		return fmt.Errorf("harness: OV_EVAL_RUN_ID not set")
	}
	iterStr := os.Getenv("OV_EVAL_ITERATION")
	var iter int
