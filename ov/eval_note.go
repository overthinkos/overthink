...
//
//   - Inside an iteration, OV_EVAL_NOTES_FILE env is set to the
...
func NotePath(projectDir, score string) string {
	if override := os.Getenv("OV_EVAL_NOTES_FILE"); override != "" {
		return override
	}
...
// substitution snapshots and the OV_EVAL_NOTES_FILE export.
...
// OV_EVAL_NOTES_FILE to be set (i.e., the caller is inside a
...
func AppendNote(projectDir, score, runID, iter, ai, text string) error {
	...
	path := os.Getenv("OV_EVAL_NOTES_FILE")
	if path == "" {
		return fmt.Errorf("note append: notes are run-scoped — only supported inside a harness iteration (no OV_EVAL_NOTES_FILE in env)")
	}
