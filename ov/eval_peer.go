...
// eval_peer.go — cross-deployment probing for `ov eval`.
//
...
//     `ov eval live` lives here (liveTargetResolver); the per-step swap is in
...
//                                     name on the shared `ov` net (ov-<name>),
...
// the same in `ov eval live`, a kind:eval bed, and an AI-iteration recipe (R3).
...
func resolvePeerVars(refs []string, instance string) (map[string]string, []func()) {
	...
	for _, key := range refs {
		...
		case peerHostVar:
			...
			// DNS name on the shared `ov` net (ov-<name>); also verifies it
			// is actually running.
...
// liveTargetResolver builds the `on:` TargetResolver used by `ov eval live`
// (and kind:eval beds, which drive `ov eval live`). For a named DRIVER
...
// cdp/wl/vnc/mcp dispatch (`ov eval cdp <method> <driver>`) connects to the
...
func liveDeployVarResolver(name, instance string, venue *EvalVenue) *EvalVarResolver {
	...
	if dc := loadDeployConfigForRead("ov eval live on:"); dc != nil {
