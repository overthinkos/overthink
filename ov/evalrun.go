...
// EvalStatus is the outcome of a single Check.
type EvalStatus int

const (
	TestPass EvalStatus = iota
	TestFail
	TestSkip
)
...
// EvalResult captures the outcome of running a single Check.
//
...
type Runner struct {
	...
	TargetResolver func(target string) (*EvalVarResolver, DeployExecutor, error)
...
// PodmanExec{ov-name}}; for RunModeImage, ImageChain(engine, ref).
...
//
// Under RunModeLive the request goes from the ov process (outside-in
...
func FormatResultsText(w io.Writer, results []EvalResult) int {
...
// Returns the number of failures.
func FormatResultsJSON(w io.Writer, results []EvalResult) int {
	type entry struct {
...
func FormatResultsTAP(w io.Writer, results []EvalResult) int {
	fails := 0
