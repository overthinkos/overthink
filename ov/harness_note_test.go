...
func TestAppendNote_HeaderAndOrdering(t *testing.T) {
	dir := t.TempDir()
	notesPath := filepath.Join(dir, ".harness", "rec", "note", "run-1.md")
	t.Setenv("OV_HARNESS_NOTES_FILE", notesPath)
...
func TestAppendNote_RequiresEnvVar(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("OV_HARNESS_NOTES_FILE", "")
	if err := AppendNote(dir, "r", "id", "1", "claude", "text"); err == nil {
		t.Error("expected error when OV_HARNESS_NOTES_FILE is unset")
	}
}

func TestAppendNote_RequiresText(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("OV_HARNESS_NOTES_FILE", filepath.Join(dir, "notes.md"))
...
func TestNotePath_DefaultsToScratchpad(t *testing.T) {
	...
	want := filepath.Join(dir, ".harness", "bench", "note", "scratchpad.md")
	if got != want {
...
func TestNotePath_RespectsEnvOverride(t *testing.T) {
	...
	t.Setenv("OV_HARNESS_NOTES_FILE", override)
