...
func TestExpandFromPod(t *testing.T) {
	...
	uf.Pod["redis"] = &PodSpec{
		Tests: []Check{fxCheckFile("redis-binary", "/usr/bin/redis-server")},
		DeployTests: []Check{fxCheckCommand("redis-ping", "redis-cli ping")},
	}
	layers := map[string]*Layer{}
	recipe := &HarnessRecipe{
		From: []HarnessRecipeFrom{
...
func TestExpandFromVM(t *testing.T) {
	...
	uf.VM["arch-vm"] = &VmSpec{
		Tests:       []Check{fxCheckCommand("vm-id", "id")},
		DeployTests: []Check{fxCheckCommand("vm-uptime", "uptime")},
	}
	layers := map[string]*Layer{}
	recipe := &HarnessRecipe{
		From: []HarnessRecipeFrom{
...
func TestMultiKindComposition(t *testing.T) {
	...
	uf.Images["img-a"] = ImageConfig{
		Tests: []Check{fxCheckCommand("img-a-test", "true")},
	}
	uf.Pod["pod-a"] = &PodSpec{
		Tests: []Check{fxCheckFile("pod-a-test", "/etc/foo")},
	}
	layers := map[string]*Layer{
...
func TestScopeFilterDeployOnly(t *testing.T) {
	...
	uf.Images["i"] = ImageConfig{
		Tests:       []Check{fxCheckCommand("img-build", "echo build")},
		DeployTests: []Check{fxCheckCommand("img-deploy", "echo deploy")},
	}
	recipe := &HarnessRecipe{
		From: []HarnessRecipeFrom{
