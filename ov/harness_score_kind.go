...
// DefaultMCPEndpoint is the canonical ov-mcp bind URL.
...
var (
	...
	ErrNoScores = errors.New("harness: no scores configured (add a 'score:' map to harness.yml)")
...
func PrintScores(w io.Writer, catalog map[string]*HarnessScore) {
	if len(catalog) == 0 {
		fmt.Fprintln(w, "No scores configured. Add a 'score:' map to harness.yml.")
		return
	}
