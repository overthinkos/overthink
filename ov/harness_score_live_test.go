...
// function returns a TestRunResults whose Scenario slice contains
...
func TestRunRecipeScenariosLive_PureCycleEmitsFailVerdictsNoPropagation(t *testing.T) {
	...
	scenarios := []Scenario{
		{Name: "a", Pod: "test-pod", SourceRecipe: "r1", DependsOn: []string{"b"},
			Steps: []Step{{Then: "x", Check: Check{File: "/a"}}}},
	...
	if res == nil {
		t.Fatalf("expected non-nil TestRunResults even on pure cycle")
	}
