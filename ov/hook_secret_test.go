...
func TestResolveHookSecretEnv(t *testing.T) {
	...
	t.Setenv("OV_TEST_HOOK_TOKEN", "tok-abc-123")
	meta := &BoxMetadata{SecretAccept: []EnvDependency{{Name: "OV_TEST_HOOK_TOKEN"}}}
	got := resolveHookSecretEnv("img", "", meta)
	want := "OV_TEST_HOOK_TOKEN=tok-abc-123"
	found := false
	for _, e := range got {
		if e == want {
			found = true
	...
	meta2 := &BoxMetadata{SecretAccept: []EnvDependency{{Name: "OV_TEST_HOOK_UNSET_XYZZY"}}}
