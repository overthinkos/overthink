...
// `ov --host <alias|user@host[:port]> <verb>` — re-exec ov on a
...
// shouldReexecForHost returns true if ov should forward the current
...
func ReexecOverSSH(cli *CLI) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "ov: --host %q: %v\n", cli.Host, err)
		return 2
	}
	...
	if err := cmd.Run(); err != nil {
		...
		fmt.Fprintf(os.Stderr, "ov: ssh %s: %v\n", target, err)
		return 1
	}
	return 0
}
