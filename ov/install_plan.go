...
// `ov deploy del` can walk them without re-compiling the plan.
...
func (s *TaskStep) Reverse() []ReverseOp {
	...
	case s.Task.Mkdir != "":
		...
		// only record paths for manual inspection via `ov deploy status`.
		return nil
	}
	return nil
}
...
type ServiceCustomStep struct {
	Name        string // e.g. "ov-ollama-ollama"
...
// them tracked separately so `ov deploy del` can reverse them precisely.
