...
func TestServicePackagedStepReverse(t *testing.T) {
	...
	s := &ServicePackagedStep{
		...
		OverridesPath: "/etc/systemd/system/postgresql.service.d/ov-pg.conf",
		PriorEnabled:  true,
	}
	ops := s.Reverse()
...
func TestServiceCustomStepRequiresGate(t *testing.T) {
	s := &ServiceCustomStep{Name: "ov-x-y", UnitText: "[Unit]\n", UnitPath: "/etc/systemd/system/ov-x-y.service", TargetScope: ScopeSystem, Enable: true}
