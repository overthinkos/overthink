...
// kubeconfig to ~/.cache/ov/clusters/<deploy>/kubeconfig.yaml.
//
...
func K3sPostProvision(deployName string) error {
	...
	retrieved := filepath.Join(home, ".cache", "ov", "clusters", safe, "kubeconfig.yaml")
	...
	fmt.Fprintf(os.Stderr, "ClusterProfile written to ~/.config/ov/clusters/%s.yaml (ingress=traefik, storage=local-path)\n", safe)
	return nil
}
