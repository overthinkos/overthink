...
`
	if err := os.WriteFile(filepath.Join(dir, "overthink.yml"), []byte(src), 0o644); err != nil {
		t.Fatalf("write overthink.yml: %v", err)
	}
...
func TestVmSshAlias(t *testing.T) {
	if got := VmSshAlias("arch-vm"); got != "ov-arch-vm" {
		t.Errorf("VmSshAlias(arch-vm) = %q, want ov-arch-vm", got)
	}
	if got := VmSshAlias("k3s-vm"); got != "ov-k3s-vm" {
		t.Errorf("VmSshAlias(k3s-vm) = %q, want ov-k3s-vm", got)
	}
}
...
func TestSshExecutor_NoCredentials(t *testing.T) {
	...
	if got := args[len(args)-1]; got != "ov-arch-vm" {
		t.Errorf("destination = %q, want ov-arch-vm", got)
	}
}
