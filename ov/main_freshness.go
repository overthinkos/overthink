...
// CheckBinaryFreshness verifies the running ov binary isn't stale relative
...
//	the build log alone; only `which ov` + `stat /usr/bin/ov` versus
//	`stat ./ov/ov` revealed the mismatch.
//
//	Per CLAUDE.md R9 ("After pushing code, explicitly rebuild on the
//	target and verify ov version. If the version is old, the fix under
...
// under any binary version, and being able to run `ov version` to
...
//
// Bypass: OV_SKIP_FRESHNESS_CHECK=1 disables the check entirely. Use
...
func CheckBinaryFreshness(verbPath string) {
	if os.Getenv("OV_SKIP_FRESHNESS_CHECK") != "" {
		return
	}
	...
	newestPath, newestModTime := newestGoFile(filepath.Join(sourceRoot, "ov"))
	if newestPath == "" {
		return
	}
	...
	fmt.Fprintf(os.Stderr, "ov: error: stale binary detected — refusing to run %q\n\n", verbPath)
	...
	fmt.Fprintf(os.Stderr, "90 minutes re-downloading 462 MiB because /usr/bin/ov predated\n")
	fmt.Fprintf(os.Stderr, "the cache-mount fix at commit 230c5d4.\n\n")
	fmt.Fprintf(os.Stderr, "Fix:    cd %s && task build:ov\n", sourceRoot)
	fmt.Fprintf(os.Stderr, "Bypass: export OV_SKIP_FRESHNESS_CHECK=1   (NOT recommended)\n")
	os.Exit(1)
}
