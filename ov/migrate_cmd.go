...
type MigrateCmdGroup struct {
	...
	MarimoRename MigrateMarimoRenameCmd `cmd:"marimo-rename" help:"Rename the marimo-ml image and marimo-ml-pod deployment to the post-cutover 'marimo' (cross-kind name reuse). Walks overthink.yml + deploy.yml + image.yml + ~/.config/ov/deploy.yml. Longest-first textual rewrite collapses marimo-ml-pod and marimo-ml to a single canonical 'marimo'. Idempotent."`
