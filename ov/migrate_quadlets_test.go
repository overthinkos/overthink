...
func TestQuadletHasMountHook(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			body: "[Service]\nExecStartPre=/usr/bin/ov config mount immich\nTimeoutStartSec=0\n",
			ref:  "immich",
			want: true,
		},
		{
			name: "bare ov binary path",
			body: "[Service]\nExecStartPre=ov config mount immich\n",
			ref:  "immich",
			want: true,
		},
		{
			name: "tolerant to leading whitespace",
			body: "[Service]\n  ExecStartPre=/home/u/.local/bin/ov config mount immich\n",
			ref:  "immich",
			want: true,
		},
		{
			name: "trailing flag tolerated",
			body: "ExecStartPre=/usr/bin/ov config mount immich --quiet\n",
			ref:  "immich",
			want: true,
		},
		{
		...
		},
		{
			...
			body: "ExecStartPre=/usr/bin/ov config mount immich-ml\n",
			ref:  "immich",
			want: false,
		},
		{
			name: "hook is for the LONGER image name when looking up the shorter",
			body: "ExecStartPre=/usr/bin/ov config mount foo-bar\n",
			ref:  "foo",
			want: false,
		},
		{
...
func TestDetectStaleEncryptedQuadlets(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(xdg, "ov"), 0o700); err != nil {
		t.Fatal(err)
	}
	t.Setenv("XDG_CONFIG_HOME", xdg)

	deployYAML := `version: 4
deploy:
  immich:
    image: immich
    target: pod
...
[Service]
Restart=always
ExecStartPre=/usr/bin/ov config mount jupyter
TimeoutStartSec=0
[Install]
WantedBy=default.target
`
...
// but never ran `ov config <name>`) is silently skipped — there's nothing
...
func TestDetectStaleEncryptedQuadlets_NoQuadletOnDisk(t *testing.T) {
	...
	t.Setenv("XDG_CONFIG_HOME", xdg)
	if err := os.WriteFile(filepath.Join(xdg, "ov", "deploy.yml"), []byte("version: 4\ndeploy:\n  ghost:\n    image: ghost\n    target: pod\n    volumes:\n      - {name: data, type: encrypted}\n"), 0o600); err != nil {
		t.Fatal(err)
	}
