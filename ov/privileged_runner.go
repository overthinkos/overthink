...
func RunPrivileged(p PrivilegedRun) error {
	...
	if p.OutputPath != "" {
		...
		// post-run copy step.
		var err error
		hostStaging, err = os.MkdirTemp("", "ov-priv-")
		if err != nil {
...
// ov settings (typically auto|machine|sudo|native). Errors are non-fatal
