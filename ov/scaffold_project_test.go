...
func TestScaffoldProject(t *testing.T) {
	...
	for _, p := range []string{"image.yml", "layers", ".gitignore"} {
		if _, err := os.Stat(filepath.Join(dir, p)); err != nil {
			t.Errorf("expected %s to exist: %v", p, err)
		}
	}
	// Idempotency: re-scaffolding the same dir should fail (we never
	// silently clobber an existing image.yml).
	if err := ScaffoldProject(dir); err == nil {
...
// top of image.yml is preserved (proves the yaml.Node API is wired
...
func TestScaffoldProject_AddImageRoundtrip(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(dir, "image.yml"))
	if err != nil {
		t.Fatalf("read: %v", err)
	}
	if !strings.Contains(string(data), "image.yml — image definitions for this project.") {
		t.Errorf("scaffold's leading comment was destroyed by AddImage; image.yml=\n%s", data)
	}
	// Confirm the structure is parseable AND the image is present.
	var root struct {
	...
	got, ok := root.Images["hello"]
	if !ok {
		t.Fatalf("hello image missing; image.yml=\n%s", data)
	}
...
func TestAddLayerToImage(t *testing.T) {
	...
	data, _ := os.ReadFile(filepath.Join(dir, "image.yml"))
...
func TestRemoveLayerFromImage(t *testing.T) {
	...
	if strings.Contains(string(data), "sshd") {
		t.Errorf("sshd should be removed; image.yml=\n%s", data)
	}
	if !strings.Contains(string(data), "tmux") {
		t.Errorf("tmux should remain; image.yml=\n%s", data)
	}
}
