...
// scaffold_scenario.go — `ov candy add-scenario`: idempotently append a
// Gherkin scenario to a layer's `description.scenario` list.
//
// This is the SPECIFY-stage authoring affordance of Agent Driven Development:
...
// `ov candy add-rpm` (idempotent append, comment-preserving yaml.Node API).
//
// Scenarios belong on the LAYER that provides the behaviour: a scenario
// authored here bakes into the org.overthinkos.description label of EVERY
...
// CandyAddScenarioCmd: `ov candy add-scenario <layer> <name> [--given … --when … --then …]`.
