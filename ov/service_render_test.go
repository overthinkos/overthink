...
{{.SystemUnitDir}}/ov-{{.Layer}}-{{.Name}}.service
{{- else -}}
{{.UserUnitDir}}/ov-{{.Layer}}-{{.Name}}.service
{{- end -}}`
...
{{.SystemUnitDir}}/{{.PackagedUnit}}.d/ov-{{.Layer}}.conf
{{- else -}}
{{.UserUnitDir}}/{{.PackagedUnit}}.d/ov-{{.Layer}}.conf
{{- end -}}`
...
func TestRenderServiceCustomSystemd(t *testing.T) {
	...
	if rendered.UnitPath != "/etc/systemd/system/ov-ollama-ollama.service" {
		t.Errorf("UnitPath = %q, want /etc/systemd/system/ov-ollama-ollama.service", rendered.UnitPath)
	}
...
func TestRenderServicePackagedWithOverrides(t *testing.T) {
	...
	want := "/etc/systemd/system/postgresql.service.d/ov-postgresql.conf"
...
func TestRenderServiceUserScope(t *testing.T) {
	...
	want := "/home/atrawog/.config/systemd/user/ov-x-x.service"
