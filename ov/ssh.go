...
// `ov ssh tunnel …` — opens an SSH-forwarded local endpoint pointing
...
// SshCmd is the top-level `ov ssh` command group.
type SshCmd struct {
...
type sshTunnelFlags struct {
	Uri string `name:"uri" env:"OV_LIBVIRT_URI" help:"Libvirt URI (default: qemu:///session). For a non-local hypervisor, use qemu+ssh://[user@]host/session."`
