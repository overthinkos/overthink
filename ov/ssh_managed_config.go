...
//
// `ov vm create` writes one Host stanza per VM into a managed file at
// ~/.config/ov/ssh_config, fenced by the same `# overthink:begin` /
...
//
// After this is in place, `ssh ov-<vmname>` works directly from any
// terminal — and ov's own SSHExecutor constructs
// `&SSHExecutor{Host: "ov-"+vmName}` with no User/Port/Key, letting
...
// Host stanza for a VM. ov vm create populates this from the VM's
// post-provision state (SSH port forward, generated key path, user).
type VmSshStanza struct {
	// Alias is the ssh-config Host alias (e.g. "ov-arch-vm"). Must be
	...
	// User is the guest account ssh logs in as ("ov", "arch", "root", …).
	User string
...
// SshFragmentPath returns ~/.config/ov/ssh_config given the user's
// home dir.
func SshFragmentPath(home string) string {
	return filepath.Join(home, ".config", "ov", "ssh_config")
}
...
// The "ov-" prefix namespaces overthink-managed aliases away from the
