...
// ForwardUnix opens a local UNIX socket under /tmp/ov-tunnel-<uuid>.sock
...
func (t *SSHTunnel) ForwardUnix(ctx context.Context, remoteSocket string) (string, func(), error) {
	...
	localPath := filepath.Join(os.TempDir(), "ov-tunnel-"+tok+".sock")
...
// with concurrent ov processes sharing /tmp.
