...
// declared `target: k8s` deployment (from the folded overthink.yml Deploy
...
// host — they emit a manifest tree that `ov deploy sync` / `kubectl apply -k`
...
// `ov eval k8s` verbs use) for the workload's readiness and refines Status
...
// declared in the projected overthink.yml. An absent unified projection plus
...
func (k *K8sCollector) Collect(ctx context.Context, opts CollectOpts) ([]DeploymentStatus, error) {
	...
	for _, name := range entries {
		...
		// `ov deploy add` has generated the Kustomize tree.
		treePresent := false
		if rootErr == nil {
...
// k8sImageRef resolves the image a k8s deploy runs, mirroring runK8s: the
// node's explicit Image, falling back to the deploy name.
...
// reusing the same client-go subset and predicates the `ov eval k8s` verbs
