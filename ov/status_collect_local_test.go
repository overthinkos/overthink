...
func TestLocalCollector_SynthesizesFromLayerRecords(t *testing.T) {
	...
	// deploy-A pulled both ripgrep + uv → 2 layers.
	...
	// deploy-B pulled only uv → 1 layer (singular label).
	if b.Image != "local (1 layer)" {
		t.Errorf("row[1].Image = %q, want local (1 layer)", b.Image)
	}
}
...
func TestLocalCollector_DeployRecordUnionNoDoubleCount(t *testing.T) {
	...
	// base + ov + sshkeys (record) + extra-layer (layer pass) = 4 distinct.
