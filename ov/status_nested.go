...
// status_nested.go — the NESTED-substrate overlay for `ov status`.
//
...
// `ov deploy add` / `ov eval live parent.child` use.
//
...
// `ov status` table/JSON/detail output — and DEDUPLICATES: a declared nested
...
func applyNestedOverlay(rows []DeploymentStatus, opts CollectOpts) []DeploymentStatus {
	...
	// row (a nested pod is also a top-level ov-<flat-path> container the pod
...
// (the flattened ov-<seg1_seg2> name) — that flat row's REAL collected data is
...
// ov-<seg1_seg2> name a nested pod carries). A row already claimed by a
...
// chain construction reuses ResolveDeployChain — the SAME primitive `ov deploy
// add` and `ov eval live parent.child` use (R3); there is no bespoke nested
// dial here.
