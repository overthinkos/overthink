...
func TestNestedOverlay_AttachesDeclaredChildren(t *testing.T) {
	...
	rows := []DeploymentStatus{
		{Kind: SubstratePod, Image: parent, Status: "running", Container: "ov-" + parent, Source: "podman"},
		{Kind: SubstratePod, Image: "redis", Status: "running", Container: "ov-redis", Source: "podman"},
	}
...
// that ALSO exists as a top-level flat row (its own ov-<flat-path> container
...
func TestNestedOverlay_MovesFlatPodRow(t *testing.T) {
	...
	rows := []DeploymentStatus{
		...
		{Kind: SubstratePod, Image: flatKey, Status: "running", Uptime: "Up 3 minutes", Container: "ov-" + flatKey, Source: "podman"},
	}
	...
	if web.Container != "ov-"+flatKey {
		t.Errorf("nested web Container = %q, want moved %q", web.Container, "ov-"+flatKey)
	}
...
func TestNestedOverlay_MovesFlatAndroidRowOnly(t *testing.T) {
	...
	rows := []DeploymentStatus{
		{Kind: SubstratePod, Image: parent, Status: "running", Container: "ov-" + parent, Source: "podman"},
		{Kind: SubstrateAndroid, Image: devicePath, Status: "online", Container: "emulator-5554", Network: "in-pod (ov-" + parent + ")", Source: "adb"},
	}
	opts := CollectOpts{Unified: nestedUnified(parent), RunMode: "quadlet"}
...
// pass through unchanged so `ov status` never regresses on a config-less host.
