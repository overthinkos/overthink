...
// came from. It is the discriminator that lets `ov status` present pod, VM,
...
// built once per `ov status` invocation by Collector.All and handed to every
...
type CollectOpts struct {
	...
	Deploy     *DeployConfig // ~/.config/ov/deploy.yml (may be nil)
	Unified    *UnifiedFile  // overthink.yml projection incl. folded kind:eval beds (may be nil)
