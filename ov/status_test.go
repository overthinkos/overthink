...
func TestParsePS_Podman(t *testing.T) {
	in := `[{"Names":["ov-ollama"],"State":"running","Status":"Up 3 hours","Ports":[{"host_ip":"127.0.0.1","container_port":11434,"host_port":11434,"range":1,"protocol":"tcp"}]}]`
	rows, err := parsePS(in)
	if err != nil {
	...
	if rows[0].Name != "ov-ollama" || rows[0].State != "running" {
...
func TestParsePS_Podman_PortRange(t *testing.T) {
	in := `[{"Names":["ov-x"],"State":"running","Status":"Up","Ports":[{"host_ip":"127.0.0.1","container_port":8000,"host_port":8000,"range":3,"protocol":"tcp"}]}]`
	rows, err := parsePS(in)
	if err != nil {
		t.Fatalf("parsePS: %v", err)
	}
...
func TestParsePS_DockerNDJSON(t *testing.T) {
	in := `{"Names":"ov-ollama","State":"running","Status":"Up 3 hours","Ports":"127.0.0.1:11434->11434/tcp"}` + "\n" +
		`{"Names":"ov-jupyter","State":"running","Status":"Up 1 hour","Ports":"127.0.0.1:8888->8888/tcp, 0.0.0.0:5900->5900/tcp"}`
	rows, err := parsePS(in)
	if err != nil {
		t.Fatalf("parsePS: %v", err)
	}
...
func TestCollector_LookupDeploy_KeyShapes(t *testing.T) {
	c := &Collector{
		deploy: &DeployConfig{
			Deployment: map[string]DeploymentNode{
