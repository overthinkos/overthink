...
func renderBootloaderScript(d *DistroDef, mnt, kernelArgs, rootfs, sshUser string) (string, error) {
	...
	if kernelArgs != "" {
		fmt.Fprintf(&b, "echo 'GRUB_CMDLINE_LINUX_DEFAULT=\"%s\"' > %s/etc/default/grub.d/00-ov-kernel-args.cfg || \\\n", kernelArgs, mnt)
		fmt.Fprintf(&b, "  { mkdir -p %s/etc/default/grub.d && echo 'GRUB_CMDLINE_LINUX_DEFAULT=\"%s\"' > %s/etc/default/grub.d/00-ov-kernel-args.cfg; }\n", mnt, kernelArgs, mnt)
