...
// vm_clone_cmd.go — Kong subcommand wiring for `ov vm clone`. The
...
// VmCloneCmd implements `ov vm clone <new> --from <src>[@<snap>]`.
type VmCloneCmd struct {
	...
	// Build, when true, also runs `ov vm build` after writing vm.yml.
	Build bool `long:"build" default:"true" help:"After writing vm.yml, run ov vm build to materialize the clone disk"`
}

// Run executes `ov vm clone`.
func (c *VmCloneCmd) Run() error {
	...
	if c.Build {
		fmt.Printf("running ov vm build %s ...\n", c.Name)
		...
		fmt.Fprintln(os.Stderr, "note: clone build path requires the source VM and snapshot to be live; rerun `ov vm build "+c.Name+"` after this clone declaration is in vm.yml")
	}
	return nil
}
