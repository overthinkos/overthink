...
func (c *VmImportCmd) Run() error {
	if c.Diff {
		if c.Domain == "" {
			return fmt.Errorf("ov vm import --diff: domain name is required")
		}
		name := c.TargetName
		if name == "" {
	...
	if c.Update {
		if c.Domain == "" {
			return fmt.Errorf("ov vm import --update: domain name is required")
		}
		name := c.TargetName
		if name == "" {
			name = stripOvPrefix(c.Domain)
...
// overthink.yml whose source.kind is "imported", and runs DiffImported
