...
// TestVmSourceClone_Validation covers the new clone-source validation
// rules added in libvirt_validate.go.
func TestVmSourceClone_Validation(t *testing.T) {
	tests := []struct {
		name      string
		src       VmSource
		shouldErr bool
		...
		{
			name: "clone with all required fields",
			src: VmSource{
				Kind:         "clone",
				FromVm:       "arch",
		...
		{
			name:      "clone missing from_vm",
			src:       VmSource{Kind: "clone", FromSnapshot: "baseline"},
			shouldErr: true,
			hint:      "from_vm",
		},
		{
			name:      "clone missing from_snapshot",
			src:       VmSource{Kind: "clone", FromVm: "arch"},
			shouldErr: true,
			hint:      "from_snapshot",
		},
		{
			name: "clone with cloud_image fields rejected",
			src: VmSource{
				Kind:         "clone",
				FromVm:       "arch",
				FromSnapshot: "baseline",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			validateVmSource("test", &tt.src, errs)
			has := errs.HasErrors()
			if has != tt.shouldErr {
				t.Errorf("validation has-errors=%v, want %v; errors=%v", has, tt.shouldErr, errs.Errors)
			}
		})
	}
}

// TestVmSourceImported_Validation covers the imported-source
// validation.
func TestVmSourceImported_Validation(t *testing.T) {
	tests := []struct {
		name      string
		src       VmSource
		shouldErr bool
		...
		{
			name: "imported with all required fields",
			src: VmSource{
				Kind:        "imported",
				LibvirtName: "my-vm",
				DiskPath:    "/var/lib/libvirt/images/my-vm.qcow2",
				DiskFormat:  "qcow2",
			},
			shouldErr: false,
		},
		{
			name:      "imported missing libvirt_name",
			src:       VmSource{Kind: "imported", DiskPath: "/x", DiskFormat: "qcow2"},
			shouldErr: true,
		},
		{
			name:      "imported with bad disk_format",
			src:       VmSource{Kind: "imported", LibvirtName: "x", DiskPath: "/x", DiskFormat: "vmdk"},
			shouldErr: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			has := errs.HasErrors()
			if has != tt.shouldErr {
				t.Errorf("has-errors=%v, want %v; errors=%v", has, tt.shouldErr, errs.Errors)
			}
		})
	}
}
...
func TestSnapshotRegistry_OnDiskFormat(t *testing.T) {
	...
	want := filepath.Join(tmpHome, ".local", "share", "ov", "vm", "ov-"+vmName, "snapshots", "registry.json")
	if path != want {
